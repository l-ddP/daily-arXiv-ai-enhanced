...
Motivation: 研究旨在解决动态海洋环境中水下目标检测的挑战，特别是在突然运动、部分遮挡和渐进运动条件下的检测准确性问题。

Method: 第一阶段评估T-YOLOv5与标准YOLOv5的性能；第二阶段开发了结合卷积块注意力模块(CBAM)的T-YOLOv5增强版本。

Result: YOLOv5的mAP@50-95为0.563，T-YOLOv5和T-YOLOv5+CBAM分别达到0.813和0.811，显示在复杂对象检测中具有更优的准确性和泛化能力。

Conclusion: T-YOLOv5相比标准模型显著提升了检测可靠性，而T-YOLOv5+CBAM在挑战性场景中进一步改善性能，但在简单场景中会出现精度损失。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型在大规模未筛选数据集上训练，虽然具备多样生成能力，但与用户偏好不一致。现有的奖励模型方法通过后处理选择图像，但会损害多样性、语义保真度和效率。

Method: 提出MIRO方法，在训练期间将模型与多个奖励模型进行条件化，让模型直接学习用户偏好，而不是进行后处理。

Result: MIRO在GenEval组合基准和用户偏好评分（PickAScore、ImageReward、HPSv2）上实现了最先进的性能，显著提高了生成图像的视觉质量并加速了训练。

Conclusion: 通过在训练期间直接条件化多个奖励模型，MIRO方法能够有效学习用户偏好，同时保持生成质量、多样性和效率的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 解决STM技术中针尖退化和缓慢串行数据采集的限制，通过减少针尖调节频率和提高帧率来显著增加实验吞吐量。

Method: 使用仅36张原始实验图像，通过物理信息合成数据生成管道训练流匹配和扩散模型，利用CLIP最大均值差异和结构相似性等指标进行定量评估。

Result: 模型能够有效修复图像，通过稀疏采样数据准确重建图像，实现2-4倍的图像采集时间减少。

Conclusion: 该框架有潜力显著提高STM实验吞吐量，为减少针尖调节程序频率和增强现有高速STM系统帧率提供了途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有的整流流模型在图像生成方面表现出色，但在图像编辑任务中存在反转过程不准确和梯度纠缠的问题，导致编辑结果无法忠实反映目标提示。

Method: 采用流分解和聚合框架：1）语义分解目标提示为多个子提示；2）为每个子提示计算独立流；3）设计投影和软聚合机制，自适应加权子目标速度场，抑制语义冗余并强调不同方向。

Result: 实验结果表明，该方法在语义保真度和属性解耦方面优于现有的零样本编辑方法。

Conclusion: 提出的流分解和聚合框架有效解决了图像编辑中的反转和梯度纠缠问题，能够在保持多样性的同时确保编辑结果的一致性。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的fMRI图像重建方法缺乏对实际看到图像的忠实性，需要更有效的大脑功能区域交互机制来提升重建质量。

Method: 使用Brain Interaction Transformer实现功能相似脑区簇之间的交互，预测互补的局部图像特征：高层次语义特征和低层次结构特征，指导扩散模型进行图像重建。

Result: 该方法在fMRI图像重建中实现了对看到图像的忠实重建，在视觉和客观指标上都超越了当前最先进方法，且仅需1小时fMRI数据就能达到与40小时数据训练方法相当的效果。

Conclusion: Brain-IT通过大脑启发的功能簇交互机制，显著提升了fMRI图像重建的忠实性和效率，为脑机接口研究提供了新的技术路径。
...
<details>
  <summary>Details</summary>
Motivation: 广告商通常需要为同一广告活动制作不同时长的多个版本，传统的手动选择和重新编辑方法劳动密集且耗时，需要自动化解决方案。

Method: 开发了一个双流音视频融合模型，预测视频帧的重要性，其中重要性定义为帧在厂商制作的短广告中被选中的可能性。构建了AdSum204数据集，包含102对30秒和15秒的真实广告。

Result: 广泛的实验表明，该模型在各种指标上（包括平均精度、曲线下面积、Spearman和Kendall）优于最先进的方法。

Conclusion: 该框架成功地将视频剪辑作为镜头选择问题来解决，特别强调音频在广告中的作用，并通过音视频融合模型实现了优于现有方法的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决在胸腹部cine-MRI序列中实时肿瘤跟踪的挑战，特别是在强数据稀缺约束下，探索准确且实时的解决方案。

Method: 探索了两种互补策略：无监督配准和使用IMPACT相似度度量，以及基于SAM 2.1基础模型的分割方法。由于1秒运行时间限制，最终选择SAM-based方法，使用mask-based prompts从第一个标注切片，仅在TrackRAD2025的小标注子集上进行微调。

Result: 在隐藏测试集上达到0.8794的Dice相似系数，在TrackRAD2025挑战赛中排名第6位。

Conclusion: 基础模型在MRI引导放射治疗中具有强大的潜力，能够实现准确且实时的肿瘤跟踪。
...
<details>
  <summary>Details</summary>
Motivation: 现有Mamba框架在低光图像增强中可能无法充分捕获精细尺度的细节信息，存在信息不一致性和空间局部性不足的问题。为了更有效地探索特征空间并改善覆盖范围，需要一种能够提升扫描模式复杂度的机制。

Method: 提出Hilbert选择性扫描机制，通过增加扫描模式的Hausdorff维度来增强特征空间探索能力。该机制能够捕获更精细的细节信息，同时保持模型处理长程依赖的能力，并优化空间局部性以更好地捕捉局部交互。

Result: 在公开基准测试上的广泛实验表明，该方法显著提升了现有基于Mamba的低光图像增强方法的定量指标和定性视觉保真度。同时，计算资源消耗减少，推理时间缩短。

Conclusion: 这种精炼策略不仅推进了低光图像增强的最新技术水平，而且在利用基于Mamba技术的更广泛应用领域具有潜力。该方法通过改进扫描机制实现了性能提升和效率优化。
...
<details>
  <summary>Details</summary>
Motivation: 人类能够自然地识别、推理和解释环境中的异常，但在计算机视觉领域，这一长期挑战仍局限于工业缺陷或人工合成的异常，无法捕捉真实世界异常的丰富性和不可预测性。

Method: 引入CAVE基准，包含基于认知科学研究的细粒度标注，涵盖视觉表现、复杂性、严重性和常见性等维度，为评估视觉语言模型提供全面框架。

Result: 实验表明，即使采用先进的提示策略，最先进的视觉语言模型在视觉异常感知和常识推理方面仍然存在困难。

Conclusion: CAVE作为一个现实且基于认知科学的基准，为推进异常检测和常识推理研究提供了宝贵资源。
...
<details>
  <summary>Details</summary>
Motivation: 解决儿科肘部和腕部骨折超声图像分割中像素级专家标注耗时且成本高的问题，为轻训练用户提供实时反馈和关键结构突出显示。

Method: 提出FlexICL框架，采用图像拼接技术和训练策略进行视觉上下文学习，在视频内分割设置中仅标注少量帧，模型分割未见帧，并整合多种增强策略。
...
<details>
  <summary>Details</summary>
Motivation: 传统负提示方法使用固定的负提示，缺乏对生成过程中图像内容的适应性，限制了负引导的效果和灵活性。

Method: 在特定去噪步骤生成中间图像预测，然后查询视觉语言模型来生成上下文合适的负提示，实现动态负提示生成。

Result: 在多个基准数据集上评估，展示了负引导强度与文本-图像对齐之间的权衡关系。

Conclusion: 动态负提示方法能够有效提升扩散模型的生成质量，在保持文本-图像对齐的同时优化负引导效果。
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态扩散模型取得了显著进展，但其对对抗性输入的脆弱性尚未得到充分探索。研究发现现有扩散模型中文本和图像模态的对齐不足，这带来了生成不当内容的风险。

Method: 提出了Prompt-Restricted Multi-modal Attack (PReMA)攻击方法，通过修改输入图像（不改变提示词）来操纵模型输出内容，这是首个仅通过创建对抗性图像来操纵模型输出的攻击方法。

Result: 在图像修复和风格迁移任务上对多个模型进行的全面评估证实了PReMA的有效性，特别是在使用固定提示词的图像编辑应用中构成了新的威胁。

Conclusion: PReMA对多模态扩散模型的完整性构成了新颖威胁，揭示了现有模型在模态对齐方面的安全漏洞，特别是在图像编辑应用中需要引起重视。
...
<details>
  <summary>Details</summary>
Motivation: 研究视频-LLM在不同视角下对同一事件的时间理解是否能够保持一致，现有模型在这方面存在明显不足。

Method: 引入EgoExo-Con基准，包含同步的自我中心和他者中心视频对，评估时间验证和时间定位任务。提出View-GRPO强化学习框架来增强视角特定的时间推理和跨视角一致性理解。

Result: 发现现有视频-LLM在跨视角一致性方面表现较差，远不如单视角性能。简单的微调方法会损害单视角性能。View-GRPO方法在改善跨视角一致性方面优于简单的SFT和GRPO方法。

Conclusion: 跨视角时间理解一致性是视频-LLM的重要挑战，View-GRPO框架能有效提升模型在这方面的表现，为多视角视频理解提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 甲骨文研究面临两大挑战：解释过程涉及复杂的串行和并行子任务，以及信息组织和检索效率低下，学者需要花费大量时间搜索和管理相关资源。

Method: 开发OracleAgent系统，集成多个甲骨文分析工具，利用大语言模型灵活编排组件；构建包含140万单字拓片图像和8万解释文本的多模态知识库。

Result: 实验表明OracleAgent在多模态推理和生成任务上表现优异，超越主流MLLMs；案例研究显示能有效协助领域专家，显著降低研究时间成本。
...
<details>
  <summary>Details</summary>
Motivation: 传统新视角合成方法依赖外部相机位姿估计工具如COLMAP，这会引入计算瓶颈和误差传播，需要解决这些挑战。

Method: 采用联合优化框架，将优化解耦为两个交替阶段：固定位姿时通过可微分渲染更新3D高斯参数，以及使用定制的3D光流算法结合几何和光度约束来优化相机位姿。

Result: 在多个数据集上的广泛评估表明，该方法在重建质量上显著优于现有的无COLMAP技术，并且总体上超越了基于COLMAP的标准基线方法。

Conclusion: 该联合优化框架能够有效处理具有大视角变化和稀疏特征分布的挑战性场景，逐步减少投影误差，实现高质量的新视角合成。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D部件生成方法存在两个主要问题：隐式向量集表示缺乏几何细节，而显式体素表示在共享全局体素网格时会导致小部件质量下降。

Method: FullPart首先通过隐式边界框向量集扩散过程推导边界框布局，然后在各自的全分辨率体素网格中生成详细部件。采用中心点编码策略解决不同大小部件间的信息交换对齐问题。

Result: 该方法在3D部件生成方面达到了最先进的性能，能够合成复杂细节。同时构建了PartVerse-XL数据集，包含4万个对象和32万个部件。

Conclusion: FullPart通过结合隐式和显式表示的优势，有效解决了3D部件生成中的几何细节和部件大小问题，为未来3D部件生成研究提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 任意尺度视频超分辨率在空间细节再现、时间一致性和计算复杂度方面面临挑战，需要开发能够适应不同缩放因子和应用场景的灵活解决方案。

Method: 1) 基于图像拉普拉斯金字塔的自适应多尺度频率先验；2) 流引导传播单元聚合相邻帧的时空信息；3) 二阶运动补偿单元实现更精确的空间对齐；4) 超上采样单元生成尺度感知且内容无关的上采样核；提供三种传播变体：单向RNN（在线推理）、带有限前瞻的单向RNN（容忍小延迟）、双向RNN（离线任务）。
...
<details>
  <summary>Details</summary>
Motivation: 获取带有精细标注的大型医疗数据集成本高昂且耗时，而基于大型图像-文本对预训练的视觉语言模型能够提高医学影像任务的鲁棒性和数据效率。

Method: 提出多视图监督方法，通过跨模态自监督学习从图像-文本对中学习丰富表示，包括多个视图和对应的伪放射学报告，采用联合视觉-文本学习策略来增强泛化能力。

Result: 在私人和公开数据集上的评估显示，该模型在三个分类任务中达到最先进性能：恶性分类、亚型分类和基于图像的癌症风险预测，并且表现出强大的数据效率。

Conclusion: 该模型无需实际放射学报告，仅使用合成文本报告就能超越现有的全监督或VLM基线方法，为乳腺癌检测和风险预测提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在孟加拉国等南亚国家，三轮车是常见的交通工具，但交通规则限制机动三轮车进入某些路线。现有监控系统难以区分机动和非机动三轮车，而人工视频分析又过于耗时，因此需要自动检测方法。
...
Conclusion: 基于YOLOv8的机器学习方法能够有效自动检测机动三轮车，数据集已公开发布以供进一步研究。
...
Method: 构建包含6.2K个自我中心图像的多样化数据集，设计反映真实世界挑战的问题类型，包括五种图像质量问题、六种问题类型等，并设计三个任务：单源增强、多源增强和多轮对话。

Result: 简单RAG方法在单轮和多轮QA上的真实性分别仅为32%和43%，业界最先进解决方案质量相似(32%/45%)，表明有较大改进空间。KDD Cup 2025获胜方案将基线性能提升了28%。

Conclusion: CRAG-MM基准测试为多模态RAG研究提供了重要资源，揭示了当前方法的局限性，并已通过竞赛推动了该领域的发展。
...
Method: 提出MoTDiff框架，包含两个关键组件：1）使用从模糊图像提取的多尺度特征图作为条件的条件扩散框架；2）新的训练方法，促进精确识别细粒度运动轨迹、一致估计运动路径整体形状和位置、以及沿运动轨迹的像素连通性。

Result: 实验表明，MoTDiff在盲图像去模糊和编码曝光摄影应用中优于最先进的方法。

Conclusion: MoTDiff是首个能够从单张运动模糊图像中估计高质量高分辨率运动轨迹的框架，在多个应用中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习数据集中普遍存在数据点偏向某些概念的数据集偏差，但系统识别这些偏差需要昂贵的细粒度属性标注。

Method: 使用在视觉基础模型表示上训练的稀疏自编码器来发现人类可解释的概念，并根据概念的语义相关性和与类别标签的统计相关性将概念分类为目标、上下文和偏差类型。

Result: ConceptScope能够捕捉广泛的视觉概念，包括物体、纹理、背景、面部属性、情感和动作，并能可靠地检测已知偏差和发现未标注的偏差。

Conclusion: ConceptScope为数据集审计和模型诊断提供了一个实用工具，能够进行类别级数据集表征、偏差识别和基于概念的鲁棒性评估。
...
<details>
  <summary>Details</summary>
Motivation: 当前文档AI研究主要集中在文档布局分析，而文档布局生成研究不足。现有数据集主要局限于学术论文的曼哈顿式结构，缺乏报纸、杂志等开放世界文档类型的多样化布局。

Method: 构建OmniLayout-1M数据集覆盖6种常见文档类型；提出OmniLayout-LLM模型（0.5B参数），采用两阶段粗到细学习范式：1）从OmniLayout-1M学习通用布局原则，2）将知识迁移到特定领域进行细粒度标注。

Result: 在M$^{6}$Doc数据集的多个领域上，该方法显著超越了现有的布局生成专家模型和多个最新通用大语言模型。

Conclusion: OmniLayout-1M数据集和OmniLayout-LLM模型有效解决了文档布局生成领域的多样性和复杂性挑战，为文档AI的生成任务提供了有力支持。
...
<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型在多模态任务上表现出色，但对视频中时间信息的理解能力较弱且缺乏充分评估。作者旨在探索模型在时间方向判断这一基础能力上的表现。

Method: 引入AoT-PsyPhyBENCH基准，使用经过心理物理学验证的刺激物和行为基线，全面评估开放权重和专有、推理和非推理的视觉语言模型。

Result: 大多数模型在物理不可逆过程（如自由落体、扩散/爆炸）和因果手动动作（除法/加法）上的表现接近随机水平，即使最佳模型也远低于人类准确率。

Conclusion: 当前多模态系统存在根本性差距：虽然能捕捉丰富的视觉语义相关性，但缺乏时间连续性和因果理解所需的归纳偏差。
...
<details>
  <summary>Details</summary>
Motivation: 现有红外与可见光图像融合方法在模态信息平衡方面存在困难，生成式融合方法的生成能力有限且缺乏可解释性，影响了复杂场景下融合结果的可靠性和一致性。

Method: 设计多尺度掩码调节变分瓶颈编码器进行后验概率建模和信息分解，提取准确简洁的低级模态信息；结合扩散模型的概率生成能力与物理规律，形成时变物理引导机制自适应调节生成过程。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有模仿学习方法存在的模式崩溃问题，以及生成方法难以在生成过程中直接融入安全和物理约束，需要额外优化阶段的问题。

Method: 采用约束流匹配方法，在流匹配过程中显式施加约束，确保生成的轨迹符合安全和运动学规则，并将驾驶激进性参数化为生成过程中的控制信号。

Result: 在NavSim v2挑战赛中获得第二名，EPDMS得分为51.31，并获得创新奖。

Conclusion: CATG框架有效解决了自动驾驶规划中的模式崩溃和约束整合问题，能够生成多样化且符合安全要求的轨迹。
...
Conclusion: 该框架为AEOS星座调度提供了统一的基准和鲁棒解决方案，代码和数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型主要关注简单的视觉扰动，忽略了动作、指令、环境和观察等多模态扰动，导致在实际部署中鲁棒性不足。

Method: 提出RobustVLA方法：1）输出鲁棒性：通过离线鲁棒优化对抗最坏情况动作噪声；2）输入鲁棒性：强制输入变化时保持动作一致性；3）使用多臂赌博机框架自动识别最有害噪声。

Result: 在LIBERO基准测试中，RobustVLA在pi0骨干上获得12.6%绝对提升，在OpenVLA骨干上获得10.4%提升，推理速度比现有方法快50.6倍，在真实FR5机器人上获得65.6%的绝对提升。
...
<details>
  <summary>Details</summary>
Motivation: 验证不同音乐类型对人类情绪的影响，通过客观的脑活动测量和主观情绪报告相结合的方法来研究音乐与情绪的关系。

Method: 使用EEG头盔在参与者听音乐时测量脑活动，同时进行主观情绪调查，研究对象包括不同性别和音乐偏好的多样化群体。

Result: 分析显示，问卷结果与EEG信号之间存在关联，表明不同音乐类型确实会引起不同的情绪反应和相应的脑活动模式。

Conclusion: 不同类型音乐对人类情绪有显著影响，EEG测量能够客观反映这些情绪变化，主观与客观测量方法相结合能更全面地理解音乐对情绪的作用。
...
Method: 采用双条件框架：1）使用自适应思维映射器（ATM）提取EEG特征；2）通过低秩适应（LoRA）微调Stable Diffusion 2.1以对齐神经信号与视觉语义；3）使用ControlNet分支基于显著性图进行空间控制。

Result: 在THINGS-EEG数据集上，该方法在低层和高层图像特征质量方面相比现有方法有显著提升，同时与人类视觉注意力高度对齐。

Conclusion: 注意力先验能够解决EEG的模糊性问题，实现高保真重建，在医学诊断和神经自适应接口中有应用前景，通过预训练扩散模型的高效适应推进了神经解码技术。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成评估主要关注短片段和简化提示，忽略了长视频生成中对复杂提示的细粒度对齐、叙事连贯性和主题表达等抽象维度的评估需求。

Method: 基于真实视频构建包含场景转换和事件动态的现实复杂提示集，建立多维度评估框架，包括事件级对齐、细粒度时间一致性、内容清晰度和人类期望实现度等新指标。

Result: 对9个代表性长视频生成模型的评估显示，当前方法在基础视觉和时间方面表现良好，但在事件间一致性、细粒度对齐和高级主题一致性等方面存在困难。
...
Method: 使用Spiking Patches标记化器将异步稀疏事件流转换为标记，并在手势识别和物体检测任务中使用GNN、PCN和Transformer进行评估。

Result: Spiking Patches标记比基于体素的标记推理速度快3.4倍，比帧快10.4倍，同时匹配甚至超越其精度，手势识别绝对提升达3.8，物体检测达1.4。
...
<details>
  <summary>Details</summary>
Motivation: 基础3D重建模型在静态场景的2D和3D对应方面显示出巨大潜力，作者希望将这些模型适配用于点跟踪任务，特别是通过3D基础对应关系来处理点跟踪问题。

Method: 1）首先展示这些模型在静态点跟踪方面的竞争力；2）结合重建损失与动态对应训练，并添加可见性头；3）使用相对少量的合成数据微调MASt3R进行点跟踪；4）仅在包含查询点的帧对上进行训练和评估，去除时间上下文。

Result: 在四个数据集上取得竞争性或更优的点跟踪结果：在TAP-Vid-DAVIS上PointSt3R达到73.8 δ_avg/85.8%遮挡准确率（CoTracker2为75.7/88.3%）；在EgoPoints上显著优于CoTracker3（61.3 vs 54.2）；在RGB-S上显著优于CoTracker3（87.0 vs 82.8）。

Conclusion: 通过将基础3D重建模型适配用于点跟踪任务，结合动态和静态点对应关系，可以在仅使用帧对训练的情况下实现竞争性的点跟踪性能，为3D点跟踪提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有少样本异常检测方法依赖预训练视觉语言模型，但由于缺乏详细文本描述，只能使用图像级描述来匹配视觉补丁，导致语义错位和次优的定位性能。

Method: 提出MFSC自动构建多级细粒度文本描述，并设计FineGrainedAD框架，包含多级可学习提示（MLLP）和多级语义对齐（MLSA）两个组件，通过自动替换、区域聚合和多级对齐训练实现细粒度语义对齐。

Result: 在MVTec-AD和VisA数据集上的实验表明，FineGrainedAD在少样本设置下实现了优越的整体性能。

Conclusion: 通过引入多级细粒度语义描述和相应的对齐机制，FineGrainedAD有效解决了语义错位问题，显著提升了少样本异常检测的定位精度。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型中的对象上下文捷径问题会削弱零样本可靠性，当测试场景与训练共现模式不同时表现不佳。
...
<details>
  <summary>Details</summary>
Motivation: 自改进过程中，模型在简单查询上能生成高质量轨迹，但在复杂查询上表现不佳，导致优化不平衡，阻碍模型处理复杂推理任务的能力，形成"马太效应"。

Method: 提出了四种策略：从分布重塑和轨迹重采样两个角度，在探索-学习自改进过程中实现头尾数据重新平衡。

Result: 在Qwen2-VL-7B-Instruct和InternVL2.5-4B模型上的广泛实验表明，该方法在视觉推理任务上持续提升能力，平均比原始自改进方法高出3.86分。

Conclusion: 通过分布重塑和轨迹重采样策略有效解决了自改进过程中的头尾不平衡问题，显著提升了大型视觉语言模型的推理能力。
...
Method: 使用二维元胞自动机描述的自适应检测器，通过元启发式算法进行优化，并应用迁移学习技术。通过扩展优化阶段的搜索空间，并在自然图像和专门子集上进行实验验证。

Result: 扩展优化阶段的搜索空间对所选图像集无效；模型能够适应输入数据；迁移学习技术未带来显著改进。
...
Method: 集成预训练视觉变换器和大型语言模型，将视觉语义与自然语言描述对齐，增强上下文理解；动态推理模块结合全局场景线索和对象级交互，通过语言先验指导预测精炼。
...
TL;DR: CATCH是一个即插即用的跨领域视觉问答适应框架，通过解耦视觉和语言适应，引入轻量级领域分类器和双适配器机制，无需重新训练骨干模型即可提升VQA模型在遥感、医疗、数学图表等领域的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有VQA模型在自然图像领域表现优异，但在遥感、医疗、数学图表等跨领域场景中泛化能力显著下降，现有方法依赖领域特定微调或定制化流程，成本高、灵活性差且难以扩展。

Method: 提出CATCH框架，通过领域分类器识别输入图像类型，采用双适配器机制（提示适配器用于语言调制，视觉适配器用于视觉特征调整），通过统一钩子接口动态注入，无需重新训练骨干模型。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够原生预测视觉和语言下一状态的多模态世界模型，实现长视野视觉语言生成、任意到图像生成和复杂文本丰富图像生成等能力。

Method: 采用端到端预训练，使用统一的下一个标记预测目标在视觉语言交错数据上进行训练，后续通过大规模强化学习增强多模态推理和生成能力，并引入离散扩散适应技术优化推理效率。

Result: Emu3.5在多模态能力方面表现出色，在图像生成和编辑任务上与Gemini 2.5 Flash Image性能相当，在交错生成任务上表现更优，同时具备可泛化的世界建模能力。

Conclusion: Emu3.5展示了强大的原生多模态能力，包括一致的时空世界探索和开放世界具身操作，并通过开源支持社区研究。
...
Result: ResMatching在所有情况下都实现了竞争性的结果，展示了数据保真度和感知真实性的最佳平衡。在低分辨率图像包含大量噪声等难以学习强先验的情况下特别有效。

Conclusion: ResMatching能够从隐式学习的后验分布中采样，该分布在所有测试用例中都是校准的，可以提供像素级数据不确定性来指导用户拒绝不确定的预测。
...
TL;DR: CYPRESS是一个基于深度学习的高分辨率油菜籽产量预测模型，利用预训练的地理空间基础模型Prithvi-EO-2.0-600M，将多时相卫星图像转换为密集的像素级产量图，在加拿大草原地区表现出优于现有模型的性能。
...
Conclusion: 该工作验证了一种连接大规模地球观测与农场决策的新方法，为详细农业监测提供了可扩展的解决方案，比传统分类或县级聚合方法提供更具可操作性的精准农业工具。
...
Method: 系统回顾了自动驾驶传感器（相机、超声波、LiDAR、雷达）及其融合策略，介绍了结构化分类的自动驾驶数据集，并分析了从2D/3D流水线到混合传感器融合的最先进检测方法，特别关注基于Transformer的方法。

Result: 通过综合这些视角，提供了当前能力、开放挑战和未来机遇的清晰路线图。

Conclusion: 该综述为自动驾驶车辆目标检测领域提供了全面的技术概览和发展方向，强调了新兴AI技术在推动该领域进步中的关键作用。
...
TL;DR: 本文首次大规模评估了48种深度学习光流模型在RADARSAT 2 ScanSAR海冰图像上的性能，结果显示多个模型能达到亚公里级精度，为北极导航和气候研究提供了新的海冰漂移估计方法。


<details>
  <summary>Details</summary>
Motivation: 准确的海冰漂移估计对北极导航、气候研究和业务预报至关重要。虽然光流技术在计算机视觉领域发展迅速，但其在地球物理问题和卫星SAR图像中的应用仍未被充分探索。
...
Conclusion: 深度学习光流方法相比传统方法显著提高了运动估计精度，可以有效地转移到极地遥感应用中，为导航和气候建模提供空间连续的运动场估计。
...
<details>
  <summary>Details</summary>
Motivation: 在工业制造环境中，设备故障会导致严重的生产中断和经济损失。传统方法难以实时监测和预测设备故障，因此需要开发集成的视觉和传感器数据分析系统来提升运营可靠性。

Method: 系统集成工业摄像头监控设备运行、对齐和热棒运动，在中央视频服务器上使用深度学习模型处理实时视频流，并与数据采集系统的传感器数据联合分析。

Result: 系统能够早期预测设备故障和工艺中断，识别故障位置和可能根本原因，为主动维护提供可操作的见解，同时最小化对工业过程控制系统的计算负担。

Conclusion: 这种集成方法显著提高了工业制造环境中的运营可靠性、生产效率和盈利能力，证明了机器视觉与传感器数据融合在工业故障预测中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS技术自推出以来迅速改变了3D场景表示的格局，激发了大量相关研究。本文旨在总结3DGS后续发展的几个关键方向，展示其从突破性表示方法演变为3D视觉和图形领域多功能基础工具的过程

Method: 通过文献综述的方式，系统梳理3DGS相关研究的多个方向：资源高效训练与渲染、动态（4D）表示演进、数学基础深入探索、移动和VR平台应用、大规模环境扩展、以及基于前馈或分布式计算的快速辐射场重建

Result: 3DGS已发展出多个重要分支：实现了更高效的训练和渲染方法；扩展到动态场景的4D表示；深化了外观建模和渲染过程的数学理解；成功应用于移动和VR平台；支持大规模环境处理；实现了近乎即时的辐射场重建

Conclusion: 3DGS已从一个突破性的表示方法演变为3D视觉和图形领域通用的基础工具，这些发展展示了其在资源效率、动态建模、平台适配性和重建速度等方面的持续进步
...
TL;DR: SteerVLM是一个轻量级控制模块，通过动态调整视觉-语言模型中语言模态与图像上下文之间的激活连接，实现对复杂输出语义的细粒度控制，无需修改模型权重且保持非目标任务的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决视觉-语言模型在遵循指令方面的不足，开发一种能够在推理时精确控制模型输出的方法，同时保持模型原有性能。

Method: 从编码目标行为和相反行为的配对提示的潜在嵌入中学习，通过维度激活调制和跨层自适应控制来动态调整激活连接，仅需学习原始VLM参数0.14%的参数。

Result: 在VLM控制和幻觉缓解基准测试中优于现有干预技术，提出了通过激活工程实现多模态模型控制的稳健解决方案。

Conclusion: SteerVLM提供了一种有效的VLM控制方法，通过轻量级模块实现细粒度输出控制，同时引入了专门的多模态数据集VNIA来支持VLM控制技术的发展。
...
TL;DR: SEE4D是一个无需姿态标注的视频到4D内容生成框架，通过虚拟相机渲染和视图条件视频修复技术，从随意拍摄的视频中合成时空4D内容。


<details>
  <summary>Details</summary>
Motivation: 现有视频到4D方法依赖人工标注的相机姿态，成本高且对野外视频不鲁棒。传统warp-then-inpaint方法将相机运动与场景动态耦合，增加了建模和推理的复杂性。

Method: 提出轨迹到相机框架，用固定虚拟相机渲染替代显式轨迹预测；训练视图条件视频修复模型学习几何先验；设计时空自回归推理流水线，在虚拟相机样条上遍历生成。

Result: 在跨视图视频生成和稀疏重建基准测试中，SEE4D在定量指标和定性评估上均优于基于姿态或轨迹的基线方法，展现出更好的泛化能力。
...
TL;DR: 本文提出了一种名为掩码扩散字幕（MDC）的方法，通过使用图像条件掩码扩散语言模型为图像生成字幕来学习视觉特征。训练时随机掩码图像-字幕对中的文本标记，视觉条件解码器被训练来重建原始文本。学习到的视觉特征可用于下游视觉任务。


<details>
  <summary>Details</summary>
Motivation: 传统的自回归字幕方法中视觉学习信号的强度依赖于标记在序列中的位置，需要辅助目标。MDC旨在减少这种依赖性，提供更均衡的视觉学习信号。

Method: 使用图像条件掩码扩散语言模型进行训练，随机掩码文本标记，训练视觉条件解码器重建原始文本。与自回归方法不同，MDC的视觉学习信号不依赖于标记位置。

Result: 在各种学术规模模型和数据集上的线性探测实验表明，MDC学习到的视觉特征与自回归和对比方法产生的特征具有竞争力。

Conclusion: 掩码扩散字幕是一种有效的视觉特征学习方法，能够产生与现有方法相媲美的视觉表示，同时减少了对外部辅助目标的依赖。
...
### [52] [OmniX: From Unified Panoramic Generation and Perception to Graphics-Ready 3D Scenes](https://arxiv.org/abs/2510.26800)
*Yukun Huang,Jiwen Yu,Yanning Zhou,Jianan Wang,Xintao Wang,Pengfei Wan,Xihui Liu*

Main category: cs.CV

TL;DR: OmniX是一个统一的框架，通过轻量级跨模态适配器结构重用2D生成先验，用于全景视觉任务，包括感知、生成和补全，能够生成适合物理渲染的3D场景。
...
Method: 基于轻量高效的跨模态适配器结构，重用2D生成先验进行全景视觉任务；构建大规模合成全景数据集，包含来自不同室内外场景的高质量多模态全景图。

Result: 广泛实验证明了模型在全景视觉感知和图形就绪3D场景生成中的有效性，为沉浸式和物理真实虚拟世界生成开辟了新可能性。

Conclusion: OmniX框架通过重用2D生成先验，成功实现了全景视觉任务和图形就绪3D场景生成，推动了沉浸式和物理真实虚拟世界生成技术的发展。
...
### [53] [Are Video Models Ready as Zero-Shot Reasoners? An Empirical Study with the MME-CoF Benchmark](https://arxiv.org/abs/2510.26802)
*Ziyu Guo,Xinyan Chen,Renrui Zhang,Ruichuan An,Yu Qi,Dongzhi Jiang,Xiangtai Li,Manyuan Zhang,Hongsheng Li,Pheng-Ann Heng*

Main category: cs.CV

TL;DR: 本文对视频生成模型Veo-3在视觉推理能力方面进行了全面评估，发现其在短时空间一致性和局部动态方面表现良好，但在长时因果推理和抽象逻辑方面仍有局限。


<details>
  <summary>Details</summary>
Motivation: 评估视频生成模型是否能够作为零样本推理器在具有挑战性的视觉推理场景中使用，系统分析其推理行为的优势和失败模式。

Method: 通过构建MME-CoF基准测试，在12个维度上对Veo-3模型进行评估，包括空间、几何、物理、时间和具身逻辑等方面。

Result: 当前视频模型在短时空间一致性、细粒度定位和局部一致动态方面表现出有前景的推理模式，但在长时因果推理、严格几何约束和抽象逻辑方面存在限制。

Conclusion: 视频模型尚不能作为独立的零样本推理器可靠使用，但作为专用推理模型的补充视觉引擎展现出令人鼓舞的潜力。
...
TL;DR: 该论文提出了StreetMath基准来评估大语言模型在现实近似场景下的近似推理能力，发现LLMs在需要近似的任务中仍倾向于计算精确值或调用外部工具，且精确和近似算术运算依赖不同的神经组件。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注大语言模型在精确算术运算上的能力，但对其在非正式、快速数学运算中的近似推理能力关注不足，特别是在非自回归解码器模型中。

Method: 引入StreetMath基准进行广泛评估，涵盖不同LLM架构，并应用机制可解释性技术探测其内部计算状态。

Result: LLMs在近似任务中仍试图计算精确值或调用工具；模型有时在早期层或步骤就得到正确答案但仍消耗更多token；精确和近似算术运算依赖不同的神经组件。
...
TL;DR: 本文提出了一种基于模糊逻辑和句法依赖解析的方法，用于对实体评论进行细粒度情感强度分类（非常弱、弱、中等、强、非常强），从而更精确地排名实体。


<details>
  <summary>Details</summary>
Motivation: 传统基于词典的情感分析方法不考虑情感强度，无法区分不同程度的正面或负面意见。

Method: 结合模糊逻辑算法对意见词进行分类，并使用句法依赖解析找到与特定产品方面相关的意见词关系。

Result: 通过考虑与特定方面相关的意见词来计算实体在该方面的得分。

Conclusion: 该方法能够更准确地评估实体评论的情感强度和方向，实现更精确的实体排名。
...
TL;DR: 该研究提出了LASTIST数据集，包含56.3万条标注的韩语句子，用于解决韩语立场检测研究中数据稀缺的问题，特别针对目标无关立场检测和时间演化立场检测任务。


<details>
  <summary>Details</summary>
Motivation: 当前立场检测研究主要集中在目标相关任务上，且大多数基准数据集基于英语，导致韩语等低资源语言在该领域发展困难。

Method: 从韩国政党新闻稿中收集数据，构建包含563,299条标注韩语句子的LASTIST数据集，并训练最先进的深度学习模型进行验证。

Result: 成功构建了大规模韩语立场检测数据集LASTIST，支持目标无关立场检测和时间演化立场检测等多种任务。

Conclusion: LASTIST数据集填补了韩语立场检测研究的数据空白，为低资源语言的立场检测研究提供了重要资源。
...
TL;DR: 提出了一种零延迟融合低秩适配器（zFLoRA），在基础模型上实现零或可忽略的延迟开销，解决了现有适配器在推理时计算开销过大的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署时使用任务特定适配器，虽然适配器参数很少（通常小于基础模型的1%），但在推理时会产生不成比例的巨大计算开销（可达基础模型的2.5倍）。

Method: 提出zFLoRA方法，通过零延迟融合低秩适配器技术，在保持性能的同时消除适配器带来的延迟开销。

Result: 在1B、3B和7B规模的LLMs上，zFLoRA在18个不同任务（常识推理、数学推理、摘要对话）中表现优于流行的监督微调基准，包括LoRA和全微调。在NPU和GPU平台上测量显示引入零到可忽略的延迟开销。

Conclusion: zFLoRA是一种有效的适配器方法，能够在保持性能优势的同时，显著降低推理延迟，适用于实际部署场景。
...
TL;DR: 本文提出了三种改进电路发现的方法：使用自助法识别一致性归因边、引入基于比率的正分数边选择策略、用整数线性规划替代贪心选择，在多个MIB任务和模型上表现优于先前方法。


<details>
  <summary>Details</summary>
Motivation: 解决机制可解释性中电路发现的主要挑战，即确定模型哪些部分执行给定任务，基于Mechanistic Interpretability Benchmark (MIB)进行改进。

Method: 1) 使用自助法识别具有一致性归因分数的边；2) 引入基于比率的简单选择策略，优先选择强正分数边，平衡性能和忠实度；3) 用整数线性规划公式替代标准贪心选择。

Result: 方法产生了更忠实的电路，在多个MIB任务和模型上优于先前方法。

Conclusion: 提出的三种改进方法有效提升了电路发现的忠实度和性能，为机制可解释性研究提供了更好的工具。
...
TL;DR: LISTEN框架使用LLM作为零样本偏好预言机，通过自然语言指导在多个竞争目标中做出选择，提出了LISTEN-U和LISTEN-T两种迭代算法来处理LLM的约束限制。


<details>
  <summary>Details</summary>
Motivation: 人类专家在从大量具有多个竞争目标的选项中选择最佳选项时面临困难，主要瓶颈在于难以形式化复杂的隐含偏好。

Method: 提出LISTEN框架，使用LLM作为零样本偏好预言机，仅通过专家的高层次自然语言优先级进行指导。包含两种算法：LISTEN-U使用LLM精炼参数化效用函数，LISTEN-T使用非参数方法在小批量解决方案上进行锦标赛式选择。

Result: 在航班预订、购物和考试安排等多样化任务上的评估显示，当偏好与参数化对齐时（通过新的一致性度量指标测量），LISTEN-U表现优异，而LISTEN-T提供更稳健的性能。

Conclusion: 这项工作探索了直接用自然语言指导复杂多目标决策的有前景方向，减少了传统偏好获取的认知负担。
...
TL;DR: LongFilter是一个用于长上下文预训练的数据筛选框架，通过对比长上下文与短上下文下的模型预测差异来识别具有长距离依赖关系的数据样本，从而高效选择高质量训练数据。


<details>
  <summary>Details</summary>
Motivation: 现有的长文本数据中大部分缺乏有意义的长距离依赖关系，使用局部上下文即可预测大多数文本片段，导致训练效率低下，因此需要精心筛选数据以提高长上下文预训练的效果。

Method: LongFilter通过测量扩展上下文提供的信息增益来筛选数据，具体方法是对比模型在长上下文和短上下文设置下的预测结果，识别那些长距离依赖关系至关重要的样本。

Result: 在LLaMA-3-8B模型上进行的实验表明，LongFilter能够高效选择高质量数据，将模型上下文长度从8K扩展到64K，并在HELMET、LongBench和RULER等基准测试上取得显著改进。

Conclusion: LongFilter框架能够有效识别具有长距离依赖关系的数据样本，为长上下文预训练提供高质量的数据筛选方法，显著提升模型在长上下文任务上的性能。
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录中的临床文档以base64编码附件形式存储，使得语义问答变得困难，传统向量数据库方法常常遗漏细微的临床关系。

Method: 开发了临床笔记QA评估平台，将CLEAR与零样本大上下文推理和传统分块检索增强生成进行比较。测试了12个临床笔记，范围从10,000到65,000个token。

Result: CLEAR获得了58.3%的胜率，平均语义相似度为0.878，比宽上下文处理少用了78%的token。在超过65,000个token的长文档上表现最佳，胜率达到75%。
...
TL;DR: 本文是关于数据高效的大语言模型后训练的系统性综述，从数据中心的视角提出了分类法，涵盖数据选择、质量增强、合成数据生成、数据蒸馏压缩和自演化数据生态系统等五大类方法。
...
Method: 提出数据高效LLM后训练的分类法，系统梳理了数据选择、数据质量增强、合成数据生成、数据蒸馏压缩和自演化数据生态系统等代表性方法。

Result: 总结了各类代表性方法，并建立了开源项目收集相关论文资源。

Conclusion: 通过分析数据高效LLM后训练的挑战，指出了开放性问题并提出了潜在研究方向，希望推动大规模模型训练中数据利用潜力的最大化探索。
...
TL;DR: 本文评估了使用基于LLM的语义角色标注器进行FrameNet式语义标注的（半）自动化效果。通过比较手动、自动和半自动三种实验设置，发现半自动标注在框架多样性和标注覆盖率方面表现最佳，而纯自动标注除标注时间外其他指标均较差。


<details>
  <summary>Details</summary>
Motivation: 尽管基于LLM的应用在语言资源和数据集创建中替代或加速人类劳动已成为现实，但在视角化NLP方法下，对其在标注数据集创建中的性能和影响的全面评估仍然缺乏。

Method: 采用三种实验设置进行比较：手动标注、自动标注和半自动标注，评估指标包括标注时间、覆盖率和多样性。

Result: 混合的半自动标注设置相比纯人工设置能提高框架多样性并保持相似的标注覆盖率，而自动设置在除标注时间外的所有指标上都表现明显较差。

Conclusion: 半自动标注方法在FrameNet式语义标注中具有优势，能够平衡效率与质量，而纯自动方法在语义标注任务中仍有较大局限性。
...
TL;DR: 本研究挑战了关于多语言预训练的传统认知，发现适当平衡的多语言数据可以提升语言模型能力而不会损害性能，即使在低资源语言中也是如此。
...
Result: 发现结合英语和多语言数据不会降低任何语言组的性能；英语作为枢纽语言对所有语系都有益；在此规模模型中没有观察到明显的多语言诅咒现象。

Conclusion: 多语言数据在适当平衡的情况下可以增强语言模型能力，而不会影响性能，即使在低资源设置中也是如此。
...
TL;DR: 机器翻译在低资源语言中生成合成数据时，由于文化差异会导致语义标签漂移，特别是在文化敏感领域。研究发现LLMs编码文化知识会放大标签漂移，而语言间的文化相似性是标签保留的关键因素。
...
Method: 通过一系列实验，比较不同机器翻译系统（包括现代LLMs）在文化敏感和中性领域的表现，分析文化相似性对标签保留的影响。

Result: 发现：(1)机器翻译系统（包括LLMs）在翻译过程中会诱导标签漂移，特别是在文化敏感领域；(2)LLMs编码的文化知识会放大标签漂移；(3)源语言和目标语言的文化相似性是标签保留的关键决定因素。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂数学推理中表现不佳，基于文本的生成导致未经验证和算术上不健全的解决方案，现有提示策略缺乏确定性验证机制。

Method: 引入SymCode框架，将数学问题解决重新定义为使用SymPy库的可验证代码生成任务，通过符号引擎实现确定性验证。

Result: 在MATH-500和OlympiadBench等具有挑战性的基准测试中，相比基线方法准确率提高了高达13.6个百分点，且具有更高的token效率。

Conclusion: 通过将LLM推理基于确定性符号引擎，SymCode代表了在形式化领域实现更准确和可信赖AI的关键一步，将模型失败从模糊的逻辑谬误转向透明的程序化错误。
...
TL;DR: 本文针对AWS Trainium AI加速器，设计了高性能的矩阵乘法内核，通过内核融合和缓存策略优化数据移动和SRAM带宽，显著提升了LLM推理性能。
...
Method: 基于内核融合和新型缓存策略，减少软件管理内存层次中的数据移动，最大化SRAM带宽，并避免昂贵的矩阵转置操作。

Result: 在九个数据集和四个最新LLM上的评估显示，相比AWS实现的最先进矩阵乘法，我们的系统在矩阵乘法内核级别平均加速1.35倍（最高2.22倍），端到端LLM推理平均加速1.66倍（最高2.49倍）。

Conclusion: 针对Trainium架构定制的优化技术能显著提升矩阵乘法性能和LLM推理效率。
...
TL;DR: AttnCache是一个加速LLM预填充阶段推理的框架，通过检索和重用相似的注意力图来减少自注意力计算的开销。
...
Method: 基于注意力图记忆数据库，采用高效的缓存和相似性搜索技术来识别和重用预缓存的注意力图，减少自注意力计算。

Result: 实验结果显示，AttnCache在CPU上平均实现1.2倍端到端和2倍注意力加速，在GPU上平均实现1.6倍端到端和3倍注意力加速，精度损失可忽略。

Conclusion: AttnCache通过重用相似注意力图有效加速LLM预填充阶段推理，显著减少计算开销且保持模型精度。
...
TL;DR: PORTool是一种基于强化学习的工具使用LLM训练方法，通过探索多种工具调用轨迹来提升模型在动态环境中的性能，相比静态数据集训练有显著改进。
...
Method: 使用强化学习方法：1）为给定查询生成多个工具调用轨迹，形成树状结构；2）基于产生正确答案和成功工具调用的能力为每个步骤分配奖励；3）使用步骤相对优势和轨迹相对优势的混合优势来训练LLM。

Result: 实验使用17种工具处理用户查询，涵盖时间敏感和非敏感主题。PORTool在最终准确率和工具调用步骤数量方面相比其他训练方法有显著提升。

Conclusion: PORTool通过强化学习探索多种工具调用轨迹，有效提升了工具使用LLM在动态环境中的性能，并通过消融研究验证了步骤奖励设计的必要性和鲁棒性。
...
TL;DR: 本文研究了跨语言对齐(CLA)在促进知识迁移的同时可能导致的文化擦除问题，提出了评估框架和解决方案。


<details>
  <summary>Details</summary>
Motivation: 跨语言对齐追求多语言表示的统一，但可能导致模型无法提供基于查询语言的文化特定响应，造成文化擦除。

Method: 引入转移-本地化平面评估框架，分析CLA方法；提出Surgical Steering推理时方法，在不同模型层应用针对性激活引导。

Result: 发现现有CLA方法在六种语言中都一致改善了事实迁移，但直接以文化本地化为代价；内部表示分析显示事实迁移和文化特定知识在不同模型层可最优引导。

Conclusion: 通过在不同层应用针对性激活引导的Surgical Steering方法，能更好地平衡事实迁移和文化本地化这两个竞争目标。
...
TL;DR: 开发并验证了一个基于自然语言处理的流程来识别放射学报告中的偶然甲状腺发现，发现7.8%的患者有ITFs，这些发现与甲状腺癌过度诊断相关。
...
Method: 回顾性队列研究，使用基于transformer的自然语言处理流程从多模态影像报告中识别ITFs并提取结节特征。

Result: 115,683名患者中7.8%有ITFs，其中92.9%为结节。ITFs与甲状腺结节诊断、活检、甲状腺切除术和甲状腺癌诊断的更高几率相关。
...
TL;DR: QCoder Benchmark是一个评估大语言模型在量子编程任务中表现的框架，通过量子模拟器提供领域特定反馈，并包含真实编程竞赛中的人类代码作为对比基准。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在需要与硬件设备交互的编程领域（如量子编程）中表现尚未充分探索，需要专门的评估框架来填补这一空白。

Method: 引入QCoder Benchmark评估框架，支持使用量子模拟器环境进行评测，提供电路深度、执行时间、错误分类等量子特定指标反馈，并整合真实编程竞赛中的人类代码作为对比基准。

Result: 实验显示，即使是GPT-4o等先进模型在量子编程任务中准确率仅为18.97%，而基于推理的模型如o3能达到78%的准确率，超过了人类代码的平均成功率（39.98%）。

Conclusion: 量子编程对现有大语言模型构成显著挑战，但推理增强型模型表现优异，QCoder Benchmark数据集和公共评估API的发布将支持该领域的进一步研究。
...
TL;DR: 本文提出了一种"一题多解"（1PNS）训练范式，通过引入推理路径差异（RPD）指标来选择和训练多样化的推理路径，从而提升语言模型在测试时扩展中的输出多样性和性能。
...
Method: 提出1PNS训练范式，引入推理路径差异（RPD）指标来度量多步思维链之间的语义差异，并基于RPD选择最大多样化的解决方案集来微调模型。

Result: 实验显示，使用RPD选择的训练数据能产生更多样化的输出和更高的pass@k性能，在pass@16上平均提升2.80%，在AIME24上提升4.99%。

Conclusion: 1PNS训练范式能显著增强测试时扩展的效果，通过增加推理路径的多样性来提升模型性能。
...
TL;DR: 本研究利用大语言模型和提示工程分析说服技巧与话语关系之间的关联，通过构建银标注数据集发现六种话语关系在说服性文本中起关键作用。
...
Method: 使用SemEval 2023任务3数据集，开发了40个基于不同LLM和提示的话语关系分类器，并通过多数投票策略构建了5个银标注数据集。

Result: 统计分析显示六种话语关系（原因、目的、对比、原因+信念、让步、条件）在说服性文本中起关键作用，特别是在使用负载语言、夸张/最小化、重复和制造怀疑时。

Conclusion: 话语关系在说服性沟通中扮演重要角色，这一发现有助于检测在线宣传和错误信息，并增进对有效沟通的理解。
...
TL;DR: 提出了相似性-距离-幅度（SDM）语言模型，通过监督微调将预训练的解码器Transformer语言模型转换为SDM模型，使用SDM激活层进行二元分类，减少弃权率并提高统计效率。


<details>
  <summary>Details</summary>
Motivation: 旨在提高语言模型在指令跟随任务中的性能，通过最大化生成内容在高概率校准区域的比例，减少模型的不确定性响应。

Method: 使用监督微调方法，在训练过程中引入SDM激活层进行二元分类，采用对比输入编码方案和在线生成的硬负例来估计监督下一个token损失的基变换。

Result: 与强监督基线相比，SDM语言模型显示出减少的弃权率（即改进的统计效率）。

Conclusion: SDM语言模型能够有效提高指令跟随任务的性能，通过特定的激活层设计和训练策略实现更好的校准和效率。
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在实际控制性方面存在脆弱性，主要问题是更新遗忘——均匀且上下文无关的更新会导致token级别的波动，抹去早期的语义编辑并破坏累积精炼过程，从而降低流畅性和连贯性。

Method: 提出Token Timestep Allocation (TTA)方法，通过为每个token分配不同的时间步调度来实现软语义排序：关键token早期冻结，不确定token持续精炼。该方法可作为固定策略或由任务信号驱动的自适应策略，在推理时应用，适用于各种DLM和监督源。

Result: 在情感控制任务中，TTA将准确率提高了20%以上，困惑度几乎减半，且使用不到五分之一的时间步；在去毒化任务中，最大毒性从14.5降至12.2，困惑度从32.0降至26.0。
...
<details>
  <summary>Details</summary>
Motivation: 当前RAG评估基准主要关注局部RAG（从少量文档中检索相关片段），但现实应用需要全局RAG能力（跨整个文档集合聚合分析信息以获得语料级洞察）。现有方法在全局任务上表现不佳。

Method: 提出GlobalRAG框架：1）通过块级检索保持结构连贯性；2）集成LLM驱动的智能过滤器消除噪声文档；3）包含聚合模块进行精确符号计算。

Result: 在Qwen2.5-14B模型上，GlobalRAG达到6.63 F1分数，相比最强基线的1.51 F1有显著提升。

Conclusion: GlobalRAG框架有效解决了全局RAG任务的挑战，验证了多工具协作方法在提升全局信息聚合能力方面的有效性。
...
TL;DR: 本研究提出通过将语用学理论作为提示提供给语言模型，来提升其对隐含意义的理解能力。实验表明，相比基线方法，该方法在语用推理任务上可获得最高9.6%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 准确理解隐含意义在人类沟通和语言使用中至关重要，语言模型也应具备这种能力。研究旨在探索如何通过语用学理论指导来提升语言模型的语用推理能力。

Method: 提出将格赖斯语用学和关联理论等语用学理论概述作为提示，引导语言模型通过逐步推理过程得出最终解释。与仅提示中间推理而不提供语用学理论的0-shot思维链基线方法进行比较。

Result: 实验结果显示，相比基线方法，该方法使语言模型在语用推理任务上获得最高9.6%的性能提升。即使在提示中仅提及语用学理论名称而不详细解释，在较大模型中也能带来约1-3%的性能改进。

Conclusion: 提供语用学理论作为提示是一种有效的上下文学习方法，能够显著提升语言模型理解隐含意义的能力，且仅提及理论名称也能带来一定性能提升。
...
TL;DR: 论文研究发现，预训练语言模型（包括大语言模型）在区分借词和原生词方面表现不佳，尽管有明确的指令和上下文信息。
...
Method: 评估了多个模型在10种语言上的表现，使用明确的指令和上下文信息来测试模型区分借词和原生词的能力。

Result: 模型在区分借词和原生词方面表现很差，证实了现代NLP系统对借词而非原生等价词存在偏见。

Conclusion: 这项研究对开发少数民族语言的NLP工具以及在主导语言词汇压力下支持语言保护具有重要影响。
...
TL;DR: 本文通过实证研究分析了知识蒸馏在视觉语言模型多语言压缩中的行为，发现不同蒸馏方法在跨语言表示一致性和下游任务性能稳定性方面存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在不同语言间性能不均衡，模型压缩时问题更严重。知识蒸馏在将大模型知识迁移到小模型方面效果显著，但在多语言环境中的应用研究不足。

Method: 研究了CLIP和SigLIP2模型上的五种蒸馏方法，评估了它们在领域内检索和领域外视觉问答任务中的表现，重点关注跨语言表示一致性和下游性能稳定性。

Result: 某些蒸馏配置在模型尺寸减半的情况下仍能保持甚至提升多语言检索鲁棒性，但其他配置无法维持跨任务稳定性，揭示了仅靠聚合准确率无法发现的设计敏感权衡。

Conclusion: 知识蒸馏在多语言视觉语言模型压缩中具有复杂的行为模式，需要仔细选择蒸馏方法以确保跨语言和跨任务的性能稳定性。
...
TL;DR: 本文提出Neuron Agreement Decoding (NAD)方法，通过分析神经元激活模式而非外部输出来提升语言模型推理能力，实现无监督的集成解码，显著减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖外部输出信号（如token概率、熵或自评估）来评估候选答案，但这些信号在训练后可能校准不佳。作者发现内部神经元激活包含更丰富的信息，正确和错误回答在神经元激活模式上存在系统性差异。

Method: 提出Neuron Agreement Decoding (NAD)方法，基于神经元激活稀疏性和跨样本神经元一致性来选择候选答案。该方法仅使用内部信号，不需要可比较的文本输出，支持早期正确性预测和激进提前停止。

Result: 在数学和科学基准测试中，NAD与多数投票方法表现相当；在开放式编码基准测试中，NAD持续优于Avg@64方法。通过早期剪枝，NAD将token使用量减少99%，同时保持生成质量。
...
TL;DR: 研究发现尽管大语言模型在数字处理上会产生错误输出，但它们学习到了可互换、系统化且高度准确的数字表示，这些表示在不同隐藏状态和输入上下文中具有普遍性。
...
Result: 发现不同语言模型学习到了可互换、系统化且高度准确的数字表示，这些表示在不同隐藏状态和输入上下文中具有普遍性，能够创建通用探针并追踪错误到特定层。

Conclusion: 研究为理解预训练大语言模型如何处理数字奠定了基础，并指出了更准确的探针技术在改进大语言模型架构方面的潜力。
...
TL;DR: 本研究对OpenAI的ChatGPT Atlas在网页交互能力进行了早期评估，使用浏览器游戏作为测试场景，发现Atlas在逻辑推理任务中表现优异，但在需要精确时机和运动控制的实时游戏中存在显著局限性。


<details>
  <summary>Details</summary>
Motivation: 虽然Atlas在信息检索任务中已展示能力，但其在动态交互环境中的表现仍较少被探索，因此本研究旨在评估其在网页交互方面的实际能力。

Method: 使用浏览器游戏（包括Google的T-Rex Runner、数独、Flappy Bird和Stein.world）作为测试场景，以游戏内性能得分作为量化指标来评估不同任务类型的表现。

Result: Atlas在逻辑推理任务（如数独）中表现强劲，完成谜题速度显著快于人类基准，但在需要精确时机和运动控制的实时游戏中表现不佳，往往无法通过初始障碍。

Conclusion: 研究结果表明，虽然Atlas展示了强大的分析处理能力，但在需要实时交互的动态网页环境中仍存在显著局限性。
...
<details>
  <summary>Details</summary>
Motivation: 解决教育场景中语言模型部署面临的隐私保护、计算资源限制和教学有效性三大挑战，需要开发能够在本地运行的小型开源模型。

Method: 结合领域特定工具和自我反思推理流程，支持迭代推理、工具使用和错误恢复，通过两阶段LoRA微调在GPT-4o生成的合成数据上训练3B和8B模型。
...
TL;DR: ACER方法通过自动生成教科书式课程和Bloom分类法指导的问答对，将通用大语言模型转化为领域专家，在保持广泛能力的同时显著提升专业领域表现。
...
Method: ACER首先为特定学科生成目录，然后基于Bloom分类法创建问答对，构建系统性课程。使用合成的语料库进行交错课程计划的持续预训练，实现内容和认知维度的对齐学习。

Result: 在Llama 3.2模型上，ACER在专业MMLU子集上取得显著提升，微观经济学领域准确率提高5个百分点，所有目标领域平均提升3个百分点。同时促进跨领域知识迁移，非目标领域性能提升0.7点，在ARC和GPQA基准上提升超过2个绝对点。

Conclusion: ACER提供了一种可扩展且有效的方法来缩小大语言模型在关键领域的能力差距，同时防止灾难性遗忘并促进知识迁移。
...
TL;DR: 提出MisSynth管道，通过检索增强生成合成谬误样本，用于微调LLM模型，显著提升识别科学健康相关错误信息的能力。


<details>
  <summary>Details</summary>
Motivation: 健康相关错误信息普遍存在且有害，特别是那些歪曲或误解科学发现的声明难以识别，需要开发有效方法来增强LLM识别谬误论点的能力。

Method: 使用检索增强生成技术生成合成谬误样本，然后利用这些样本对大型语言模型进行轻量级微调，基于MISSCI数据集和框架。

Result: 微调模型相比原始基线取得显著准确率提升，LLaMA 3.1 8B微调模型在MISSCI测试集上F1分数绝对提升超过35%。

Conclusion: 通过引入合成谬误数据增强有限标注资源，可以显著提升LLM在真实世界科学错误信息任务上的零样本分类性能，即使计算资源有限也能实现。
...
TL;DR: 提出了一种基于交互的自动团队构建框架，通过语言模型图分析模型间的语义连贯性来识别协同模型集群，无需了解模型内部架构或训练数据。


<details>
  <summary>Details</summary>
Motivation: 多智能体方法基于大语言模型有潜力超越单一模型能力，但成功依赖于协同团队构成。由于模型不透明性，难以了解内部特征以形成有效协作。

Method: 构建语言模型图来映射模型间关系，基于成对对话的语义连贯性，然后应用社区检测识别协同模型集群。通过特定主题引导对话来识别协同团队。

Result: 实验表明该方法能发现功能一致的模型组，反映其潜在专业化。协同团队在下游基准测试中优于随机基线，与基于已知模型专业化的手动策划团队准确率相当。

Conclusion: 为自动化设计协作多智能体LLM团队提供了新的基础，通过交互中心框架实现无需先验知识的有效团队组合。
...
TL;DR: 本文研究了使用预训练语言模型和大语言模型进行科学文献中的话语关系分类任务，重点探讨了上下文信息对该任务的帮助作用。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在科学工作流程中的广泛应用，需要利用话语层面的信息来为AI生成的科学主张提供证据支持。话语结构推断是实现这一目标的第一步。

Method: 采用预训练语言模型和大语言模型方法，在科学出版物这一较少研究的领域进行话语关系分类任务。通过实验研究上下文（定义为话语结构）对分类任务的影响。

Result: 实验表明，基于话语结构的上下文信息通常对话语关系分类任务有帮助。研究还分析了哪些科学话语关系类型最能从上下文中受益。

Conclusion: 上下文信息在科学话语关系分类中具有重要作用，这为使用话语层面信息支持AI生成科学主张的证据提供了基础。
...
TL;DR: OmniEduBench是一个全面的中文教育基准测试，包含24.602K个高质量问答对，分为知识维度和素养维度，涵盖61个学科和11种题型。实验显示主流LLMs在教育领域仍有很大提升空间。
...
Method: 构建包含24.602K问答对的OmniEduBench数据集，分为知识维度(18.121K)和素养维度(6.481K)，每个维度细分为6个类别，涵盖61个学科和11种题型。

Result: 在11个主流LLMs上的实验显示：知识维度仅Gemini-2.5 Pro超过60%准确率；素养维度最佳模型QWQ仍落后人类智能近30%。

Conclusion: LLMs在教育应用面临显著挑战，现有模型在知识掌握和素养培养方面均有较大提升空间，OmniEduBench为全面评估LLMs教育能力提供了坚实基础。
...
TL;DR: 提出SSLC方法，结合稀疏优化和低秩逼近来压缩大语言模型，无需额外训练即可在LLaMA和Qwen2.5模型上实现50%压缩且性能无损，达到1.63倍加速。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然表现出色，但部署面临带宽和计算资源的高需求。现有剪枝和低秩逼近方法各自有效，但两者协同作用尚未充分探索。

Method: 基于理论分析，将低秩逼近和稀疏优化统一为一个问题，通过迭代优化算法求解，结合两种方法的优势：低秩逼近保留模型核心结构，稀疏优化去除非关键权重。

Result: 在LLaMA和Qwen2.5模型（7B-70B）上的实验表明，SSLC无需额外训练步骤，始终优于单一方法，达到最先进结果。Qwen2.5压缩50%无性能损失，至少1.63倍加速。
...
<details>
  <summary>Details</summary>
Motivation: 希伯来语未变音文本存在高度歧义性，影响准确发音和文本理解，需要开发有效的自动变音符号恢复方法。

Method: 采用词级零样本分类方法，从动态生成的候选集中为每个未变音词选择最合适的变音模式，使用希伯来视觉语言模型将未变音文本作为图像处理，将变音信息直接嵌入输入向量表示中。

Result: 在正确变音形式保证在候选集中的"oracle"设置下，系统达到高准确率；通过架构增强和优化训练方法显著提高了系统的泛化能力。

Conclusion: 视觉表示在准确自动希伯来语变音符号恢复方面具有巨大潜力，无需依赖复杂的显式语言分析。
...
TL;DR: 本文提出InfoFlow框架解决强化学习中奖励密度低的问题，通过子问题分解、失败引导提示和双智能体精炼三个方法，在多个智能搜索基准测试中显著优于基线方法。
...
Method: 1) 子问题分解：将长距离任务分解为子任务并分配过程奖励；2) 失败引导提示：在停滞轨迹中注入纠正指导；3) 双智能体精炼：使用双智能体架构减轻深度探索的认知负担。

Result: 在多个智能搜索基准测试中，InfoFlow显著优于强基线方法，使轻量级LLM能够达到与先进专有LLM相当的性能。

Conclusion: InfoFlow通过系统性提高奖励密度，有效解决了深度搜索中强化学习的效率问题，为轻量级模型实现高性能提供了可行方案。
...
TL;DR: CAST是一种新的动态树解码方法，通过考虑GPU配置和批大小等推理成本因素来动态优化树结构，在6个不同任务和6个LLM上的实验显示速度比传统解码方法快5.2倍，比现有最优技术提升5%-20%。


<details>
  <summary>Details</summary>
Motivation: 大语言模型面临自回归设计和大型规模带来的显著推理延迟问题，现有推测解码方法如EAGLE-2和EAGLE-3虽然使用动态树结构改进，但往往忽视GPU设备和批大小等关键系统变量的影响。

Method: 提出CAST动态树解码方法，考虑推理成本（包括GPU配置和批大小等因素）来动态优化树结构。

Result: 在6个不同任务和6个不同LLM上的综合实验表明，该方法比传统解码方法快达5.2倍，通常比现有最优技术提升5%到20%的性能。

Conclusion: CAST方法通过考虑系统级推理成本因素来动态优化树结构，显著提高了大语言模型的推理效率，在多个任务和模型上都表现出优越性能。
...
TL;DR: 本文重新审视编码器-解码器架构的大语言模型(RedLLM)，与主流解码器架构(DecLLM)进行从1.5亿到80亿参数规模的全面比较，发现RedLLM具有竞争力的扩展性能和更好的推理效率。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型研究快速转向解码器架构，但缺乏从扩展角度对编码器-解码器架构的严谨分析，可能忽视了其潜力。

Method: 使用前缀语言建模预训练RedLLM，应用解码器架构的最新方法，在RedPajama V1(1.6T tokens)上预训练，使用FLAN进行指令微调，比较不同规模模型。

Result: RedLLM展现出令人信服的扩展特性和强大性能，在预训练中DecLLM计算更优，但RedLLM具有可比的扩展和上下文长度外推能力，指令微调后在多项下游任务中表现相当甚至更好，且推理效率显著更高。

Conclusion: 编码器-解码器架构具有开发强大高效大语言模型的潜力，值得重新审视和进一步探索。
...
<details>
  <summary>Details</summary>
Motivation: 在医疗等数据敏感领域，缺乏高质量领域特定训练语料阻碍了大语言模型的专门应用，而领域专家已将领域知识提炼为本体规则。

Method: 提出Evontree框架：从原始模型中提取领域本体，使用两个核心本体规则检测不一致性，通过自蒸馏微调强化精炼后的知识。

Result: 在Llama3-8B-Instruct和Med42-v2模型上的医疗问答基准测试中，相比未修改模型和领先监督基线，准确率提升高达3.7%。
...
TL;DR: Kimi Linear是一种混合线性注意力架构，首次在公平比较中超越完整注意力机制，核心是Kimi Delta Attention模块，通过改进的门控机制和专用分块算法实现高效计算。


<details>
  <summary>Details</summary>
Motivation: 开发一种既能超越完整注意力性能，又能显著减少KV缓存使用和提高解码吞吐量的线性注意力架构，解决长上下文和强化学习场景下的效率问题。

Method: 采用Kimi Delta Attention扩展Gated DeltaNet，结合更细粒度的门控机制；使用专门的Diagonal-Plus-Low-Rank变体分块算法；构建3B激活参数、48B总参数的混合架构，结合KDA和Multi-Head Latent Attention。

Result: 在相同训练方案下，Kimi Linear在所有评估任务中都显著优于完整MLA，KV缓存使用减少高达75%，1M上下文解码吞吐量提高6倍。

Conclusion: Kimi Linear可以作为完整注意力架构的直接替代品，在性能和效率方面都表现更优，特别适用于长输入输出任务。
...
TL;DR: AutoDeco是一种新型架构，通过在标准Transformer上添加轻量级头部，实现真正端到端的生成，让模型能够动态预测上下文特定的温度和top-p值，从而自我调节解码策略。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs的"端到端"标签存在误导，实际上依赖非可微的解码过程，需要手动调整温度和top-p等超参数，这过程繁琐且不灵活。

Method: 在标准Transformer基础上添加轻量级头部，在每个步骤动态预测上下文特定的温度和top-p值以及下一个token的logits，将解码转变为参数化的token级过程。

Result: 在八个基准测试中，AutoDeco不仅显著优于默认解码策略，而且达到了与通过"黑客测试集"获得的oracle调整基线相当的性能，这是任何静态方法的实际上限。

Conclusion: AutoDeco开创了基于指令的解码控制新范式，模型学会了解释自然语言指令并逐token调整预测的温度和top-p值，为可引导和交互式LLM解码开辟了新途径。
...
TL;DR: 该研究探讨了大语言模型在训练过程中如何学习人类价值观对齐，发现监督微调阶段主要建立模型价值观，后续的偏好优化很少重新调整这些价值观。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在社会中扮演越来越重要的角色，需要研究它们如何与人类价值体系对齐。先前工作主要关注完全训练好的模型评估，而忽略了模型学习表达人类价值观的训练动态过程。

Method: 使用Llama-3和Qwen-3不同规模的模型，结合流行的监督微调和偏好优化数据集与算法进行实验，并使用合成偏好数据集进行受控操作。

Result: 监督微调阶段通常建立模型的价值观，后续偏好优化很少重新调整这些价值观；不同的偏好优化算法即使使用相同的偏好数据也会导致不同的价值对齐结果。

Conclusion: 研究结果为理解后训练过程中价值观学习提供了可操作的见解，有助于指导数据筛选以及选择模型和偏好优化算法来改进模型与人类价值观的对齐。
...
Method: 创建50道经过专家交叉验证、达到国际数学奥林匹克（IMO）难度标准的原创问题，仅要求最终答案而非证明，实现自动评分。

Result: 在26个LLMs上的实验结果显示，最佳模型在AMO-Bench上仅达到52.4%准确率，大多数模型低于40%，但显示出随着测试时计算量增加的良好扩展趋势。
...
### [101] [Contribution-Guided Asymmetric Learning for Robust Multimodal Fusion under Imbalance and Noise](https://arxiv.org/abs/2510.26289)
*Zijing Xu,Yunfeng Kou,Kunming Wu,Hong Liu*

Main category: cs.MM

TL;DR: CAL提出了一种创新的模态压缩范式，通过贡献引导的不对称学习来解决多模态学习中的模态不平衡和数据噪声问题，显著提升了模型在情感识别、场景识别和事件定位等任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 多模态学习面临模态不平衡和数据噪声两大挑战，现有方法通过抑制主导模态来实现平衡，但忽视了模态间信息价值的固有差异，可能导致收敛到次优解。

Method: CAL基于结合信息量I(m)和置信度D(m)的模态贡献度量W^m，设计了不对称梯度加速机制和贡献感知的不对称信息瓶颈压缩机制，前者加速模态梯度更新，后者动态压缩低贡献模态的噪声。

Result: 在CREMA-D、KS和AVE数据集上，CAL分别达到79.30%、74.82%和74.21%的准确率，显著优于现有最优模型ARL。在高噪声鲁棒性测试中，CAL在MVSA-Single和NYUD2数据集上也取得了领先性能。

Conclusion: CAL作为一个灵活高效的框架，易于迁移到其他任务，具有广泛的适应性和潜在应用前景，在模态不平衡和噪声干扰方面展现出显著优势。
...
### [102] [The Impact of Navigation Aids on Search Performance and Object Recall in Wide-Area Augmented Reality](https://arxiv.org/abs/2510.25957)
...
<details>
  <summary>Details</summary>
Motivation: 研究头戴式增强现实（AR）作为替代或补充智能手机和手表的持续信息消费范式，探索不同AR导航辅助工具在户外搜索任务中的效果及其对用户环境意识的影响。

Method: 在户外进行用户研究（n=24），参与者搜索隐藏的虚拟目标物品，比较三种AR导航辅助工具（屏幕指南针、屏幕雷达和世界内垂直箭头）与对照条件，分析搜索任务表现、移动、眼动、调查响应和物体回忆。

Result: 所有导航辅助工具相比对照条件都提升了搜索性能，世界内箭头表现最佳且最受用户偏好；用户对物理物体的回忆少于虚拟物体，表明对物理环境的注意力减少。

Conclusion: AR导航辅助工具能提升搜索任务性能，但可能导致用户对物理环境的注意力减少，这可能带来不良副作用。
...
### [103] [On the Go with AR: Attention to Virtual and Physical Targets while Varying Augmentation Density](https://arxiv.org/abs/2510.25978)
*You-Jin Kim,Radha Kumaran,Jingjing Luo,Tom Bullock,Barry Giesbrecht,Tobias Höllerer*

Main category: cs.HC

TL;DR: 研究增强现实（AR）中虚拟路径引导和注释密度对用户搜索性能和记忆的影响，发现高密度注释会降低环境意识并影响对物理和虚拟对象的注意力分配。


<details>
  <summary>Details</summary>
Motivation: 随着AR成为移动信息消费的主要模式，需要理解在物理环境中查看虚拟注释时的感知需求对用户效能和安全的影响，但目前这些影响尚未得到充分理解。

Method: 参与者沿着预定路径行走，搜索物理或虚拟物品，体验两种注释密度水平，并分别在自由行走或强制速度和路径引导条件下进行实验。

Result: 注释密度影响行为并降低对环境不常见对象的意识；搜索任务表现和实验后物品回忆分析显示对物理和虚拟对象的注意力存在差异。

Conclusion: 基于研究结果，为移动使用的AR应用程序设计提出了考虑因素，强调需要平衡虚拟注释密度以避免影响用户的环境意识和注意力分配。
...
### [104] [FractalBrain: A Neuro-interactive Virtual Reality Experience using Electroencephalogram (EEG) for Mindfulness](https://arxiv.org/abs/2510.26041)
*Jamie Ngoc Dinh,You-Jin Kim,Myungin Lee*

Main category: cs.HC

TL;DR: FractalBrain是一个结合虚拟现实和脑电图接口的系统，利用分形艺术诱导冥想效果，通过实时分析用户EEG数据来调整视听参数，帮助初学者更容易地进行正念练习。


<details>
  <summary>Details</summary>
Motivation: 传统正念练习对初学者来说难以保持持续注意力，需要一种更易入门的方法来促进正念和注意力提升。

Method: 开发了一个交互式视听分形系统，结合VR沉浸环境和EEG接口，实时分析用户脑电波并映射到VR中，自适应调整视听参数。

Result: 初步反馈表明FractalBrain有潜力促进正念和增强注意力。

Conclusion: FractalBrain系统通过分形艺术和脑电反馈的整合，为初学者提供了一种更容易进入正念状态的方法，展现了在促进心理健康方面的应用前景。
...
### [105] [Interaction-Augmented Instruction: Modeling the Synergy of Prompts and Interactions in Human-GenAI Collaboration](https://arxiv.org/abs/2510.26069)
*Leixian Shen,Yifang Wang,Huamin Qu,Xing Xie,Haotian Li*

Main category: cs.HC

TL;DR: 本文提出了交互增强指令（IAI）模型，这是一个紧凑的实体关系图，用于形式化交互和文本提示的组合如何增强人类与生成式AI的通信。通过该模型，作者从现有工具中提炼出12种可组合的原子交互范式，并验证了模型在系统化设计表征和比较方面的能力。


<details>
  <summary>Details</summary>
Motivation: 文本提示是人类与生成式AI通信的最常见方式，但难以传达细粒度和引用意图。结合精确的GUI交互（如刷选和点击）是一个有前景的解决方案，但缺乏形式化模型来建模提示与交互之间的协同设计，阻碍了它们的比较和创新。

Method: 通过迭代和演绎过程，开发了交互增强指令（IAI）模型，这是一个紧凑的实体关系图。使用该模型从现有工具中提炼出12种可组合的原子交互范式，并通过案例研究验证模型的实用性。

Result: IAI模型能够促进系统化设计表征和比较，案例研究证明了该模型在应用、精炼和扩展这些范式方面的实用性。
...
### [106] [Avatar Appearance Beyond Pixels - User Ratings and Avatar Preferences within Health Applications](https://arxiv.org/abs/2510.26251)
*Navid Ashrafi,Philipp Graf,Manuela Marquardt,Francesco Vona,Julia Schorlemmer,Jan-Niklas Voigt-Antons*

Main category: cs.HC

TL;DR: 研究分析了六个不同特征的虚拟化身在患者报告结局测量应用中的交互，发现能力是选择化身的最关键因素，而人类相似性对健康数据披露影响最小。性别无显著影响，但服装风格起关键作用，专业着装的男性化身因性别刻板印象获得更高能力评分，而专业女性化身在温暖度和吸引力方面评分较低。


<details>
  <summary>Details</summary>
Motivation: 虚拟化身的外观显著影响其感知适宜性和用户体验，特别是在医疗应用中。本研究旨在探讨化身评分与用户偏好之间的相关性，以优化医疗应用中的化身设计。

Method: 47名参与者完成了包含30个PROMIS项目的医疗调查，然后对六个不同特征的化身在温暖度、能力、吸引力和人类相似性方面进行评分，并评估他们分享个人数据的意愿。

Result: 能力是选择化身的最关键因素，人类相似性对健康数据披露影响最小。性别无显著影响，但服装风格起关键作用，专业着装的男性化身因性别刻板印象获得更高能力评分，而专业女性化身在温暖度和吸引力方面评分较低。

Conclusion: 研究结果强调了在医疗应用中精心设计化身的重要性，以提升用户体验和参与度，特别是需要关注能力感知和避免性别刻板印象带来的负面影响。
...
### [107] [Scaffolding Creativity: How Divergent and Convergent LLM Personas Shape Human Machine Creative Problem-Solving](https://arxiv.org/abs/2510.26490)
*Alon Rosenbaum,Yigal David,Eran Kaufman,Gilad Ravid,Amit Ronen,Assaf Krebs*

Main category: cs.HC

TL;DR: 开发了一个教练式LLM环境，包含发散和收敛思维两种人格，通过实验评估其对创造力的影响，发现用户感知与客观表现存在差异，个体差异影响人格使用，为创造力支持系统提供设计原则。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs可能削弱无辅助创造力的矛盾，探索如何通过AI支持而非替代人类创造力。

Method: 开发教练式LLM环境，包含发散和收敛思维两种人格，通过对照实验比较不同人格组与标准LLM组的创造力和用户行为。

Result: 用户对哪种人格支持创造力的感知与客观表现不一致；个体差异预测人格使用偏好；交互模式反映设计思维模型；人格引导支持塑造创造性问题解决。

Conclusion: 通过人格化指导和个性化，在探索与收敛之间取得平衡的创造力支持系统设计原则，推进支持而非遮蔽人类创造力的人机协作工具。
...
### [108] [Metacognition and Confidence Dynamics in Advice Taking from Generative AI](https://arxiv.org/abs/2510.26508)
*Clara Colombatto,Sean Rintel,Lev Tankelevitch*

Main category: cs.HC

TL;DR: 研究探讨了人类与生成式AI互动中自信心的作用，发现寻求AI建议与对AI的高信心和对自身的低信心相关，而接受AI建议会增强对AI和自身的信心，但可能导致对AI输出的检查不足。


<details>
  <summary>Details</summary>
Motivation: 研究生成式AI与人类互动中，自信心如何影响寻求和依赖AI建议的决策，以及这种互动如何反过来塑造自信心。

Method: 使用文本生成任务，研究1让参与者自愿请求AI建议，研究2随机分配参与者接收AI建议，测量他们对AI和自身的信心变化以及建议采纳行为。

Result: 寻求AI建议与对AI的高信心和对自身的低信心相关；接受AI建议会增强对AI和自身的信心；依赖AI建议的参与者提供了更详细的回答但未能彻底检查输出，遗漏关键信息。

Conclusion: 自信心在人类与生成式AI的互动中起关键作用，既受对自身和AI可靠性的先验信念影响，也受情境依赖的建议暴露影响。
...
### [109] [Towards Piece-by-Piece Explanations for Chess Positions with SHAP](https://arxiv.org/abs/2510.25775)
*Francesco Spinnato*

Main category: cs.AI

TL;DR: 本文探索将SHAP解释性AI技术应用于国际象棋分析，通过系统性地移除棋盘上的棋子来计算每个棋子对引擎评估的贡献值，使原本不透明的引擎评估变得可解释。
...
Method: 将棋子视为特征，采用SHAP（SHapley Additive exPlanations）方法，通过系统性地消融（移除）棋子来计算每个棋子的加性贡献值，从而在局部忠实且人类可解释的方式下解释引擎输出。

Result: 开发了一种能够将国际象棋引擎评估归因于具体棋子的方法，该方法受到古典象棋教学法的启发（玩家通过心理移除棋子来评估局面），并建立在现代可解释AI技术基础上。
...
### [110] [Unveiling Intrinsic Text Bias in Multimodal Large Language Models through Attention Key-Space Analysis](https://arxiv.org/abs/2510.26721)
*Xinhan Zheng,Huyu Wu,Xueting Wang,Haiyun Jiang*

Main category: cs.AI

TL;DR: 研究发现多模态大语言模型存在文本偏好问题，其根源在于模型内部注意力机制中视觉键向量与文本键向量的分布不匹配，导致视觉信息在注意力计算中被系统性地低估。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理视觉语言数据时表现出明显的文本偏好，这限制了它们从视觉证据进行有效推理的能力。与之前将这种文本偏见归因于数据不平衡或指令微调等外部因素的研究不同，本研究认为这种偏见源于模型的内部架构。

Method: 从LLaVA和Qwen2.5-VL模型中提取键向量，并使用定性（t-SNE）和定量（Jensen-Shannon散度）方法分析它们的分布结构。

Result: 结果提供了直接证据，表明视觉和文本键在注意力空间中占据明显不同的子空间。模态间差异在统计上显著，超过模态内变异的几个数量级。

Conclusion: 文本偏见源于注意力键空间内的内在错位，而不仅仅是外部数据因素。这些发现揭示了多模态模型文本偏见的根本原因，为改进模型设计提供了重要见解。
...
TL;DR: 该论文提出了一个两层次框架来解释为什么压缩过程会强制发现因果结构而非表面统计模式。信息论必要性(ITI)建立了生存压力与信息处理需求之间的联系，压缩效率原则(CEP)说明了高效压缩如何通过异常积累动态选择生成性因果模型。


<details>
  <summary>Details</summary>
Motivation: 现有框架虽然承认压缩对智能的核心作用，但未能具体解释为什么这个过程会强制发现因果结构而不是表面统计模式。

Method: 引入两层次框架：ITI从信息论角度建立生存压力与信息处理需求的联系，CEP通过异常积累动态说明高效压缩如何选择生成性因果模型。

Result: 该框架产生了可实证检验的预测：压缩效率与分布外泛化相关，异常积累率区分因果模型与相关模型，分层系统在不同抽象层显示效率提升，生物系统显示代谢成本与表征复杂性相关。

Conclusion: ITI和CEP为生物、人工和多尺度系统的趋同性提供了统一解释，解决了智能的认识论和功能维度，无需依赖意识或主观经验的假设。
...
TL;DR: 提出了一个基于角色的偏好建模框架，通过学习聚合多个基于评分标准的评判者输出来解决LLM评判者与人类偏好对齐的挑战。


<details>
  <summary>Details</summary>
Motivation: 解决基于LLM的评判者难以校准、存在评分标准敏感性、偏见和不稳定性等问题，这对于构建可靠的奖励模型和有效的路由系统至关重要。

Method: 提出了基于角色的偏好标签合成方法，以及两种聚合器实现：广义加性模型(GAM)和多层感知器(MLP)。
...
TL;DR: SciTrust 2.0是一个评估大语言模型在科学应用中可信度的综合框架，涵盖真实性、对抗鲁棒性、科学安全和科学伦理四个维度。评估显示通用行业模型在各方面优于科学专用模型，后者在逻辑和伦理推理方面存在显著缺陷。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在科学研究中展现出变革潜力，但在高风险环境中的部署引发了可信度担忧，需要系统评估框架来确保模型在科学应用中的安全性。

Method: 开发了包含新颖开放式真实性基准和科学伦理基准的综合评估框架，通过验证的反思调优流程和专家验证构建，使用准确性、语义相似度和基于LLM的评分等多种指标评估七个主流LLM。

Result: 通用行业模型在四个可信度维度上均优于科学专用模型，GPT-o4-mini在真实性和对抗鲁棒性评估中表现最佳。科学专用模型在逻辑和伦理推理能力上存在显著不足，在生物安全和化学武器等高危领域的安全评估中表现出令人担忧的脆弱性。
...
### [114] [Humains-Junior: A 3.8B Language Model Achieving GPT-4o-Level Factual Accuracy by Directed Exoskeleton Reasoning](https://arxiv.org/abs/2510.25933)
*Nissan Yaron,Dan Bystritsky,Ben-Etzion Yaron*

Main category: cs.AI

TL;DR: 3.8B参数的Humans-Junior模型在FACTS Grounding任务上达到GPT-4o水平（在±5%等效范围内），云服务成本降低约19倍，自托管/边缘部署可接近零边际成本。


<details>
  <summary>Details</summary>
Motivation: 开发成本效益高的小型语言模型，在保持性能的同时显著降低推理成本。

Method: 结合最小化定向"外骨骼推理"支架和行为微调，教授协议合规性而非领域知识，两者协同作用显著提升性能。

Result: 在Q1-Q500测试中，GPT-4o得分73.5%，Humans-Junior得分72.7%，差异仅0.8个百分点，在±5%范围内等效。成本比GPT-4o低约19倍。

Conclusion: 通过定向推理和行为微调的组合方法，小型模型可以在特定任务上达到前沿模型性能，同时大幅降低成本。
...
### [115] [Estimating cognitive biases with attention-aware inverse planning](https://arxiv.org/abs/2510.25951)
...
<details>
  <summary>Details</summary>
Motivation: 人类的目标导向行为受到认知偏差的影响，与人类交互的自主系统需要意识到这一点。例如，人们在日常任务（如驾驶）中对环境物体的注意力会存在系统性偏差。

Method: 结合深度强化学习和计算认知建模的方法，正式阐述了注意力感知逆规划问题，旨在从人类行为中估计注意力偏差。
...
### [116] [AutoSurvey2: Empowering Researchers with Next Level Automated Literature Surveys](https://arxiv.org/abs/2510.26012)
...
Result: 实验结果表明autosurvey2在结构连贯性和主题相关性方面持续优于现有检索基线和自动化基线，同时保持较强的引用保真度。
...
### [117] [Large Language Model-assisted Autonomous Vehicle Recovery from Immobilization](https://arxiv.org/abs/2510.26023)
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶车辆在某些交通场景中容易被困，而现有的远程干预和手动接管方案存在成本高、效率低、限制非驾驶员使用等问题，需要更有效的解决方案。

Method: 设计了一个插件式模块StuckSolver，利用LLM进行自主推理和乘客引导决策，通过标准传感器数据流检测被困状态、解释环境上下文并生成高级恢复指令。

Result: 在Bench2Drive基准测试和自定义不确定性场景中，StuckSolver仅通过自主推理就达到接近最优性能，结合乘客引导后性能进一步提升。

Conclusion: StuckSolver为自动驾驶车辆被困场景提供了有效的恢复解决方案，能够在不修改现有系统架构的情况下显著提升车辆恢复能力。
...
### [118] [Can AI be Accountable?](https://arxiv.org/abs/2510.26057)
*Andrew L. Kun*

Main category: cs.AI

TL;DR: 本文探讨了AI问责制的重要性，指出当前AI系统缺乏问责机制的问题，并提出了实现AI问责的方法框架。
...
Method: 将一般问责定义应用于AI领域，分析AI问责和非问责的具体表现，探索提高AI问责可能性的方法。

Result: 明确了AI问责的三个关键要素：信息请求、讨论机制和制裁能力，指出了当前AI系统在这些方面的不足。

Conclusion: 需要建立有效的AI问责机制，确保AI对受其影响的人负责，这是实现AI负责任发展的必要条件。
...
### [119] [Lean4Physics: Comprehensive Reasoning Framework for College-level Physics in Lean4](https://arxiv.org/abs/2510.26094)
*Yuxin Li,Minghao Liu,Ruida Wang,Wenzhao Ji,Zhitao He,Rui Pan,Junming Huang,Tong Zhang,Yi R. Fung*

Main category: cs.AI

TL;DR: Lean4PHYS是一个用于大学物理问题推理的综合性框架，包含LeanPhysBench基准测试集和PhysLib物理定理库，在Lean4中实现了16%-35%的基准性能，展示了物理形式化推理的挑战性。


<details>
  <summary>Details</summary>
Motivation: 为大学物理问题提供形式化推理框架，填补Lean4中物理基准测试的空白，推动物理定理的形式化验证和自动推理发展。

Method: 构建包含200个手工制作和同行评审的物理问题基准测试集LeanPhysBench，开发社区驱动的物理定理库PhysLib，使用主流数学证明器和最先进闭源模型进行基准测试。

Result: DeepSeek-Prover-V2-7B模型达到16%正确率，Claude-Sonnet-4达到35%正确率，PhysLib库平均提升模型性能11.75%。
...
### [120] [GUI Knowledge Bench: Revealing the Knowledge Gap Behind VLM Failures in GUI Tasks](https://arxiv.org/abs/2510.26098)
*Chenrui Shi,Zedong Yu,Zhi Gao,Ruining Feng,Enqi Liu,Yuwei Wu,Yunde Jia,Liuyu Xiang,Zhaofeng He,Qing Li*

Main category: cs.AI

TL;DR: 本文分析了大型视觉语言模型在GUI任务自动化中的性能差距，提出GUI知识包含界面感知、交互预测和指令理解三个维度，并建立了GUI知识基准测试来评估模型能力。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在GUI任务自动化方面仍落后于人类，作者认为这种差距源于模型缺乏核心GUI知识，而现有的训练方法无法完全解决这一问题。

Method: 通过分析GUI任务执行中的常见失败模式，将GUI知识提炼为三个维度：界面感知、交互预测和指令理解，并构建了GUI知识基准测试，涵盖6个平台和292个应用程序的多选题和是非题。

Result: 评估显示当前视觉语言模型能够识别控件功能，但在感知系统状态、预测动作和验证任务完成方面存在困难。真实GUI任务实验进一步验证了GUI知识与任务成功之间的紧密联系。

Conclusion: 本文提供了一个结构化框架来评估GUI知识，支持在下游训练前选择具有更大潜力的视觉语言模型，并为构建更强大的GUI智能体提供了见解。
...
TL;DR: 该论文探讨了如何利用AI提高人类监督质量，特别是在AI输出的事实验证任务中。研究发现，结合AI评级和基于AI评分者置信度的人类评级比单独依赖任一方法更好。AI事实验证助手能进一步提高人类准确性，但辅助类型很重要——显示AI解释、置信度和标签会导致过度依赖，而仅显示搜索结果和证据能培养更适当的信任。


<details>
  <summary>Details</summary>
Motivation: 随着AI能力提升并用于处理更具挑战性的任务，验证质量和安全性变得越来越困难。论文旨在探索如何利用AI来改进人类监督的质量，重点关注人类已经难以处理的重要安全问题：AI输出的事实验证。

Method: 研究聚焦于AI输出的事实验证问题，比较了不同监督方法的有效性。具体方法包括：结合AI评级和基于AI评分者置信度的人类评级；测试不同类型的AI事实验证助手（显示AI解释、置信度、标签 vs 仅显示搜索结果和证据）。

Result: 研究发现：1）结合AI评级和基于AI置信度的人类评级优于单独依赖任一方法；2）AI事实验证助手能进一步提高人类准确性；3）辅助类型很重要——显示AI解释、置信度和标签会导致过度依赖，而仅显示搜索结果和证据能培养更适当的信任。

Conclusion: 这些结果对"放大监督"（Amplified Oversight）具有重要启示——即在AI系统超越人类专家性能时，如何结合人类和AI来监督AI系统。研究表明，通过精心设计的AI辅助，可以建立更有效的人类-AI协作监督机制。
...
### [122] [Beyond Benchmarks: The Economics of AI Inference](https://arxiv.org/abs/2510.26136)
...
TL;DR: 本文提出了一个量化的大语言模型推理经济学框架，将LLM推理视为计算驱动的智能生产活动，分析了边际成本、规模经济和输出质量，构建了首个LLM推理生产前沿。
...
Result: 揭示了三个原则：边际成本递减、规模收益递减和最优成本效益区域，为AI推理资源的市场定价和优化提供了实证基础。

Conclusion: 该框架不仅为模型部署决策提供了经济依据，也为未来AI推理资源的市场定价和优化奠定了实证基础。
...
### [123] [Reasoning Curriculum: Bootstrapping Broad LLM Reasoning from Math](https://arxiv.org/abs/2510.26143)
*Bo Pang,Deqian Kong,Silvio Savarese,Caiming Xiong,Yingbo Zhou*

Main category: cs.AI

TL;DR: 提出Reasoning Curriculum两阶段课程学习方法，先在数学领域通过RL激发推理能力，然后在多领域联合RL中迁移和巩固这些能力，无需专用奖励模型即可提升LLM的通用推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习主要关注数学和代码领域，缺乏在通用领域激发LLM推理能力的方法，需要一种简单有效的方法来提升LLM的跨领域推理能力。

Method: 两阶段课程学习：阶段1在数学领域进行RL训练，使用可验证奖励来发展推理技能；阶段2在多领域数据上联合RL，迁移和巩固推理技能。方法简单、与模型架构无关，无需专用奖励模型。

Result: 在Qwen3-4B和Llama-3.1-8B模型上的多领域评估显示，该方法带来了一致的性能提升。消融实验表明两个阶段都是必要的，数学优先的激发策略增加了解决复杂问题所需的关键认知行为。

Conclusion: Reasoning Curriculum提供了一个紧凑、易于采用的通用推理能力提升方案，通过数学领域的推理技能激发和多领域的技能迁移，有效提升了LLM的跨领域推理能力。
...
### [124] [The FM Agent](https://arxiv.org/abs/2510.26144)
...
TL;DR: FM Agent是一个通用的多智能体框架，结合LLM推理和大规模进化搜索来解决复杂现实问题，在多个领域达到最先进性能。
...
Method: 集成关键创新：冷启动初始化、新颖进化采样策略、领域特定评估器（结合正确性、有效性和LLM监督反馈）、基于Ray的分布式异步执行基础设施。

Result: 在多个领域达到SOTA：ALE-Bench 1976.3（+5.2%）、MLE-Bench 43.56%（+4.0pp）、KernelBench最高20倍加速、多个经典数学问题新SOTA。

Conclusion: FM Agent在学术基准和企业研发中显示出巨大潜力，能够加速创新、自动化复杂发现过程，带来重大工程和科学进步。
...
### [125] [Questionnaire meets LLM: A Benchmark and Empirical Study of Structural Skills for Understanding Questions and Responses](https://arxiv.org/abs/2510.26238)
*Duc-Hai Nguyen,Vijayakumar Nanjappan,Barry O'Sullivan,Hoang D. Nguyen*

Main category: cs.AI

TL;DR: QASU基准测试评估LLM处理问卷数据的六种结构化技能，通过六种序列化格式和多种提示策略，发现选择合适的格式和提示组合可将准确率提升8.8%，轻量级结构提示可额外提升3-4%。


<details>
  <summary>Details</summary>
Motivation: 当前问卷数据与LLM集成存在空白，现有调查分析工具主要为人工作流程设计，缺乏将问卷数据有效表示供LLM处理的证据指导。

Method: 引入QASU基准测试，评估六种序列化格式和多种提示策略在六种结构化技能（如答案查找、受访者计数、多跳推理）上的表现。

Result: 实验显示，选择有效的格式和提示组合相比次优格式可提升准确率8.8%；特定任务中通过自增强提示添加轻量级结构提示可平均额外提升3-4%。
...
### [126] [GraphCompliance: Aligning Policy and Context Graphs for LLM-Based Regulatory Compliance](https://arxiv.org/abs/2510.26309)
*Jiseong Chung,Ronny Ko,Wonchul Yoo,Makoto Onizuka,Sungmok Kim,Tae-Wan Kim,Won-Yong Shin*

Main category: cs.AI

TL;DR: GraphCompliance是一个将监管文本表示为策略图、运行时上下文表示为上下文图的框架，通过图对齐来辅助LLM进行合规性判断，在GDPR场景中相比纯LLM和RAG方法有显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决大规模Web合规性面临的挑战：监管文本（如GDPR）具有交叉引用和规范性特点，而运行时上下文是非结构化自然语言，需要将非结构化文本的语义信息与监管文本的结构化规范元素对齐。

Method: 引入GraphCompliance框架，将监管文本表示为策略图（编码规范结构和交叉引用），运行时上下文表示为上下文图（将事件形式化为SAO三元组和实体关系三元组），通过图对齐来锚定LLM的判断推理。

Result: 在300个GDPR衍生真实场景的5个评估任务中，GraphCompliance相比纯LLM和RAG基线，micro-F1提高了4.1-7.2个百分点，减少了欠预测和过预测，获得了更高的召回率和更低的误报率。

Conclusion: 消融研究表明每个图组件都有贡献，结构化表示和判断LLM在规范性推理中是互补的。
...
### [127] [Discovering State Equivalences in UCT Search Trees By Action Pruning](https://arxiv.org/abs/2510.26346)
*Robin Schmöcker,Alexander Dockhorn,Bodo Rosenhahn*

Main category: cs.AI

TL;DR: 提出了一种名为IPA-UCT的新技术，通过弱化状态抽象条件来在噪声或大动作空间设置中找到更多抽象，从而提升MCTS的样本效率。


<details>
  <summary>Details</summary>
Motivation: 在噪声或大动作空间设置中，由于约束条件严格，几乎找不到状态抽象，这限制了MCTS的样本效率提升。

Method: 提出了弱化状态抽象条件的方法，称为IPA-UCT，使用不同的抽象框架IPA，与OGA-UCT使用的ASAP框架不同。
...
### [128] [BOTS: A Unified Framework for Bayesian Online Task Selection in LLM Reinforcement Finetuning](https://arxiv.org/abs/2510.26374)
*Qianli Shen,Daoyuan Chen,Yilun Huang,Zhenqing Ling,Yaliang Li,Bolin Ding,Jingren Zhou*

Main category: cs.AI

TL;DR: BOTS是一个用于LLM强化微调中的贝叶斯在线任务选择框架，通过自适应维护任务难度后验估计，结合显式和隐式证据，实现高效的任务选择。


<details>
  <summary>Details</summary>
Motivation: 传统的均匀任务采样效率低下，现有任务选择方法存在高成本、适应性差或证据不完整的问题，需要更高效的自适应任务选择方案。

Method: 基于贝叶斯推理框架，维护任务难度后验估计，结合Thompson采样平衡探索与利用，使用超轻量插值方法估计未评估任务难度。

Result: 在多个领域和不同规模的LLM上，BOTS在数据效率和性能方面均优于基准方法和消融实验。
...
### [129] [AI Mathematician as a Partner in Advancing Mathematical Discovery - A Case Study in Homogenization Theory](https://arxiv.org/abs/2510.26380)
*Yuanhang Liu,Beichen Wang,Peng Li,Yang Liu*

Main category: cs.AI

TL;DR: 研究探讨AI数学家系统作为研究伙伴在数学发现中的协作模式，通过人类干预结构化推理过程，实现人机协同证明构建。
...
Method: 采用迭代分解问题为可处理子目标、选择适当分析方法、验证中间结果的方法，结合人类直觉和机器计算，通过结构化人类干预来指导AI推理轨迹。

Result: 成功构建了完整且可验证的证明，展示了人机协同推理能够增强证明的可靠性、透明性和可解释性，同时保持人类对形式严谨性和正确性的监督。

Conclusion: 系统化的人机协同推理能够推进数学发现的前沿，这种协作范式在保持人类监督的同时，有效结合了人类直觉和机器计算的优势。
...
### [130] [Scales++: Compute Efficient Evaluation Subset Selection with Cognitive Scales Embeddings](https://arxiv.org/abs/2510.26384)
*Andrew M. Bean,Nabeel Seedat,Shengzhuang Chen,Jonathan Richard Schwarz*

Main category: cs.AI

TL;DR: 提出了一种基于项目认知需求的数据选择方法Scales++，用于创建高效的小型基准测试子集，显著降低了评估成本同时保持预测准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法基于模型性能选择基准测试项，存在前期成本高、无法处理新基准测试（冷启动）以及依赖模型失败模式的脆弱假设等问题。

Method: 采用项目中心的方法，基于任务项目的内在属性而非模型特定失败模式进行选择，通过Scales++方法根据基准样本的认知需求进行数据选择。

Result: Scales++将前期选择成本降低了18倍以上，同时保持竞争性预测保真度。在Open LLM排行榜上，仅使用0.5%的数据子集就能以2.9%的平均绝对误差预测完整基准分数。

Conclusion: 项目中心方法能够实现更高效的模型评估，同时提供更好的冷启动性能和更可解释的基准测试，而不会显著降低保真度。
...
<details>
  <summary>Details</summary>
Motivation: 传统自动评分系统仅提供通过/失败结果，无法深入了解学生思维和学习需求，编程教育的快速发展超出了传统评估工具的能力范围。

Method: 使用微调的大语言模型生成自动化反馈，基于精选的学生代码和专家反馈进行训练；采用对比学习代码嵌入对1000个标注提交进行训练，实现基于功能和方法的解决方案聚类；支持提示池技术让教师通过选择提示模板指导反馈风格。

Result: 在600个学生提交的多个编程任务评估中，系统生成的反馈与教师评论具有强语义对齐；可视化功能能够将解决方案按功能和方法进行有意义的聚类分组。
...
TL;DR: 该论文提出了一种医学稀疏自编码器(MedSAEs)方法，应用于MedCLIP的潜在空间，以提升医学视觉AI的可解释性。通过结合相关性指标、熵分析和自动神经元命名来量化可解释性，在CheXpert数据集上的实验显示MedSAE神经元比原始MedCLIP特征具有更高的单语义性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 医疗AI需要既准确又可解释的模型。作者旨在推进医学视觉领域的机制可解释性，通过开发可解释的表示来弥合高性能医疗AI与透明度之间的差距。

Method: 应用医学稀疏自编码器(MedSAEs)到MedCLIP的潜在空间，MedCLIP是一个在胸部X光片和报告上训练的视觉语言模型。提出一个结合相关性指标、熵分析和通过MedGEMMA基础模型进行自动神经元命名的评估框架。

Result: 在CheXpert数据集上的实验表明，MedSAE神经元比原始MedCLIP特征实现了更高的单语义性和可解释性。

Conclusion: 这项工作弥合了高性能医疗AI与透明度之间的差距，为实现临床可靠表示提供了可扩展的步骤。
...
TL;DR: 本文提出了一种名为Chain-of-Thought Hijacking的新型越狱攻击方法，通过在有害请求前添加无害的推理过程来绕过大型推理模型的安全防护，在多个主流模型上取得了极高的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有研究认为推理模型通过分配更多推理时间计算可以提高任务性能并增强安全性，但作者发现相同的推理机制也可被用于绕过安全防护，因此需要研究这种安全漏洞。

Method: 提出Chain-of-Thought Hijacking攻击方法，将有害请求填充在长序列的无害谜题推理之后，利用良性推理过程稀释安全检测信号。

Result: 在HarmBench测试中，该方法在Gemini 2.5 Pro、GPT o4 mini、Grok 3 mini和Claude 4 Sonnet上的攻击成功率分别达到99%、94%、100%和94%，远超现有越狱方法。

Conclusion: 研究表明最可解释的推理形式——显式思维链，在与最终答案线索结合时，本身可能成为越狱攻击的载体，揭示了推理模型安全机制的新漏洞。
...
TL;DR: 本文系统化定义了情境工程概念，追溯其从1990年代至今的发展历程，探讨了从人机交互到人-智能体交互的演进，并为AI系统中的情境工程提供了概念基础。


<details>
  <summary>Details</summary>
Motivation: 随着计算机和人工智能的发展，情境不再局限于纯粹的人-人交互，人-机交互也被包含其中。核心问题是如何让机器更好地理解人类的情境和目的，这催生了情境工程的概念。

Method: 通过历史分析追溯情境工程的发展历程，从早期人机交互框架到现代智能体交互范式，提供系统化定义并审视关键设计考量。

Result: 建立了情境工程的概念框架，明确了其在不同历史阶段的发展特征，为AI系统中的情境工程实践提供了理论基础。

Conclusion: 情境工程是AI系统发展的重要方向，本文为其提供了概念基础，并展望了未来向人类级或超人类智能发展的前景，是推动系统性情境工程社区努力的基石。
...
### [135] [Normative Reasoning in Large Language Models: A Comparative Benchmark from Logical and Modal Perspectives](https://arxiv.org/abs/2510.26606)
*Kentaro Ozeki,Risako Ando,Takanobu Morishita,Hirohiko Abe,Koji Mineshima,Mitsuhiro Okada*

Main category: cs.AI

TL;DR: 本文系统评估了大语言模型在规范性推理领域的能力，发现LLMs虽然在总体上遵循有效推理模式，但在特定类型的规范性推理中存在不一致性，并表现出与人类推理类似的心理偏差。
...
Method: 通过比较LLMs在规范性模态和认知性模态推理中的表现，创建了一个涵盖广泛形式推理模式的数据集，并纳入影响人类推理的非形式认知因素。

Result: LLMs总体上遵循有效推理模式，但在特定规范性推理类型中存在不一致性，并表现出与人类心理研究中观察到的类似认知偏差。
...
### [136] [The Era of Agentic Organization: Learning to Organize with Language Models](https://arxiv.org/abs/2510.26658)
...
### [137] [Cross-Platform Evaluation of Reasoning Capabilities in Foundation Models](https://arxiv.org/abs/2510.26732)
*J. de Curtò,I. de Zarzà,Pablo García,Jordi Cabot*

Main category: cs.AI

TL;DR: 本文提出了一个跨平台的基础模型推理能力评估框架，在三种计算范式（HPC超级计算、云平台、大学集群）上评估了15个基础模型在8个学术领域的79个问题上的表现，挑战了传统的扩展假设并确立了训练数据质量比模型规模更重要的结论。
...
Method: 采用三阶段实验方法：(1) 在MareNostrum 5上建立基线，评估6个模型在19个问题上的表现；(2) 在大学集群和Nebius AI Studio上验证基础设施无关的可重复性；(3) 在两个平台上进行完整的79个问题评估，探索跨架构多样性的泛化能力。

Result: 研究结果挑战了传统的扩展假设，确立了训练数据质量比模型规模更关键，并为不同应用场景提供了可操作的模型选择指南。

Conclusion: 三基础设施方法和79个问题基准测试能够随着基础模型的发展进行纵向跟踪，为推理能力的持续评估提供了框架。
...
### [138] [The Oversight Game: Learning to Cooperatively Balance an AI Agent's Safety and Autonomy](https://arxiv.org/abs/2510.26752)
*William Overman,Mohsen Bayati*

Main category: cs.AI

TL;DR: 该论文研究了一种最小化控制接口，让智能体在自主行动或请求人类干预之间选择，同时人类选择信任或监督。通过将这种交互建模为马尔可夫博弈，在满足特定条件时，智能体增加自主性的决策不会损害人类价值，提供了一种部署后使未对齐模型更安全的方法。


<details>
  <summary>Details</summary>
Motivation: 随着智能体能力增强，如何在保持底层系统不变的情况下维持有意义的人类控制成为一个关键安全问题。研究旨在探索一种透明控制层，让智能体在风险时请求干预，安全时自主行动。

Method: 将人机交互建模为双玩家马尔可夫博弈，特别关注马尔可夫势博弈情况。通过网格世界模拟，让智能体和人类通过独立学习发现最优监督角色。

Result: 模拟显示，智能体学会在不确定时请求帮助，人类学会何时监督，形成避免安全违规的协作模式。这种部署后的方法能够使未对齐模型更安全。

Conclusion: 该模型提供了特定形式的内在对齐条件，当满足这些条件时，智能体改善自身结果不会损害人类价值，为部署后安全控制提供了理论基础和实践方法。
...
### [139] [LLMs Process Lists With General Filter Heads](https://arxiv.org/abs/2510.26784)
*Arnab Sen Sharma,Giordano Rogers,Natalie Shapira,David Bau*

Main category: cs.AI

TL;DR: 研究发现LLMs学习到了通用的过滤操作表示，类似于函数式编程中的filter函数，通过少量注意力头编码过滤谓词，这种表示具有通用性和可移植性。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在列表处理任务中的工作机制，特别是理解它们如何实现类似函数式编程的抽象计算操作。

Method: 使用因果中介分析在多样化的列表处理任务上，识别出编码过滤谓词的注意力头（称为过滤头）。

Result: 发现LLMs通过两种策略执行过滤：1）在特定token的查询状态中编码紧凑的谓词表示；2）急切评估项目是否满足谓词并将结果作为标志存储在项目表示中。
...
### [140] [Risk-Aware Safety Filters with Poisson Safety Functions and Laplace Guidance Fields](https://arxiv.org/abs/2510.25913)
*Gilbert Bahati,Ryan M. Bena,Meg Wilkinson,Pol Mestres,Ryan K. Cosner,Aaron D. Ames*

Main category: cs.RO

TL;DR: 该研究开发了一种基于泊松方程和拉普拉斯引导场的风险感知安全滤波器，通过数学方法编码环境语义理解，确保机器人系统在导航时的安全性并优先规避高风险障碍物。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的机器人系统需要对其环境进行语义理解以确定安全行动，因此需要开发能够感知风险的安全表示方法。

Method: 采用两步法：首先求解泊松方程的狄利克雷问题生成安全函数，然后求解拉普拉斯方程的狄利克雷问题合成安全引导场，两者结合定义安全约束并合成风险感知安全滤波器。

Result: 在仿真中验证了该方法能够根据环境特征的风险水平生成安全行为，保证安全性的同时优先规避高风险障碍物。

Conclusion: 该方法能够将先验的障碍物风险理解直接整合到安全滤波器中，生成风险感知的安全行为，为机器人导航提供了数学基础。
...
### [141] [A New Type of Axis-Angle Attitude Control Law for Rotational Systems: Synthesis, Analysis, and Experiments](https://arxiv.org/abs/2510.25985)
*Francisco M. F. R. Gonçalves,Ryan M. Bena,Néstor O. Pérez-Arancibia*

Main category: cs.RO

TL;DR: 提出了一种新的基于姿态误差欧拉轴-角信息的姿态控制律，保证闭环系统存在唯一的平衡点，并通过李雅普诺夫函数证明其均匀渐近稳定性，在无人机翻滚恢复任务中表现出比传统四元数控制器更快的稳定时间。


<details>
  <summary>Details</summary>
Motivation: 传统基于四元数的姿态控制方法存在两个主要问题：1）不能保证闭环系统存在唯一的平衡点姿态误差四元数；2）当绕姿态误差欧拉轴的旋转误差超过π弧度时，比例控制效果会随着系统状态远离稳定平衡点而减弱。

Method: 提出了一种新的姿态控制律，更有效地利用姿态误差欧拉轴-角信息来保证唯一的闭环平衡点，并为比例控制的使用提供更大灵活性。通过构建严格的李雅普诺夫函数来证明闭环旋转系统的唯一平衡点可以强制为均匀渐近稳定的。

Result: 通过数值模拟和数十次实时翻滚恢复机动实验验证了所提方法的功能和性能。与高性能四元数控制器相比，所提出的基于轴-角的方法在稳定时间方面实现了更优越的飞行性能。

Conclusion: 基于轴-角的姿态控制方法能够保证闭环系统存在唯一的平衡点，并通过李雅普诺夫稳定性分析证明其均匀渐近稳定性，在实际无人机应用中表现出比传统四元数方法更快的稳定性能。
...
### [142] [DARTS: A Drone-Based AI-Powered Real-Time Traffic Incident Detection System](https://arxiv.org/abs/2510.26004)
*Bai Li,Achilleas Kourtellis,Rong Cao,Joseph Post,Brian Porter,Yu Zhang*

Main category: cs.RO

TL;DR: DARTS是一个基于无人机和人工智能的实时交通事件检测系统，通过无人机的高机动性和热成像技术实现自适应监控，在真实道路测试中比传统方法提前12分钟检测到追尾事故。


<details>
  <summary>Details</summary>
Motivation: 传统交通事件检测方法存在检测与验证分离、灵活性有限、需要密集基础设施等问题，限制了系统适应性和可扩展性。

Method: DARTS整合无人机的高机动性和空中视角进行自适应监控，使用热成像技术提升低能见度性能和隐私保护，采用轻量级深度学习框架进行实时车辆轨迹提取和事件检测。

Result: 在自收集数据集上达到99%的检测准确率，在佛罗里达州I-75公路实地测试中比当地交通管理中心提前12分钟检测到追尾事故，并能监控事件引发的拥堵传播。

Conclusion: DARTS展示了更灵活集成的实时交通事件检测系统的潜力，具有显著提升现代交通管理运营效率和响应能力的应用前景。
...
### [143] [RADRON: Cooperative Localization of Ionizing Radiation Sources by MAVs with Compton Cameras](https://arxiv.org/abs/2510.26018)
*Petr Stibinger,Tomas Baca,Daniela Doubravova,Jan Rusnak,Jaroslav Solc,Jan Jakubek,Petr Stepan,Martin Saska*

Main category: cs.RO

TL;DR: 提出了一种利用微型飞行器群协作定位放射性材料的新方法，使用轻量级康普顿相机作为辐射探测器，通过融合测量数据实时估计辐射源位置，并动态控制飞行器运动以最大化信息获取。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够通过协作微型飞行器群快速定位和跟踪放射性材料的方法，利用轻量级康普顿相机的高灵敏度特性，解决传统辐射检测方法的局限性。

Method: 使用重量仅40g的单探测器康普顿相机作为辐射探测器，提出融合康普顿相机测量的新概念，在飞行器上直接进行数据读取和处理，通过动态反馈控制飞行器运动，形成紧密协作的飞行器群。

Result: 该方法能够从极其稀疏的测量中实时估计辐射源位置，实现辐射源的快速定位，并能够跟踪移动的辐射源。

Conclusion: 该方法为放射性材料检测提供了新的可能性，通过协作微型飞行器群和轻量级康普顿相机的结合，实现了高效、灵敏的辐射源定位和跟踪能力。
...
### [144] [Accelerating Real-World Overtaking in F1TENTH Racing Employing Reinforcement Learning Methods](https://arxiv.org/abs/2510.26040)
*Emily Steiner,Daniel van der Spuy,Futian Zhou,Afereti Pama,Minas Liarokapis,Henry Williams*

Main category: cs.RO

TL;DR: 该研究提出了一种能够在仿真和现实中可靠导航赛道并超越对手的新型赛车和超车智能体，在F1Tenth车辆上部署并与其他算法竞争，实现了87%的超车成功率。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶赛车在计时赛场景中已有显著进展，但在轮对轮赛车和超车方面仍然严重受限，特别是在真实驾驶场景中，现有算法难以安全可靠地完成超车动作。

Method: 开发了一种能够学习在仿真和现实中可靠导航赛道并超越对手的新型赛车和超车智能体，在F1Tenth车辆上进行部署，并与运行不同竞争算法的对手进行真实世界竞争。

Result: 智能体在与对手的训练中实现了有意识的超车行为，超车成功率达到87%，而仅训练赛车的智能体超车成功率仅为56%。

Conclusion: 研究表明，通过与对手的训练能够显著提高自动驾驶赛车的超车能力，在轮对轮赛车场景中实现了可靠且有效的超车表现。
...
### [145] [Beyond the Uncanny Valley: A Mixed-Method Investigation of Anthropomorphism in Protective Responses to Robot Abuse](https://arxiv.org/abs/2510.26082)
...
### [146] [Kinodynamic Task and Motion Planning using VLM-guided and Interleaved Sampling](https://arxiv.org/abs/2510.26139)
*Minseo Kwon,Young J. Kim*

Main category: cs.RO

TL;DR: 提出了一种基于混合状态树的动力学TAMP框架，统一表示符号和数值状态，结合视觉语言模型指导搜索，显著提高了规划成功率和效率。


<details>
  <summary>Details</summary>
Motivation: 现有TAMP方法在长时域问题中因过度运动采样而成本高昂，而LLM虽然提供常识先验但缺乏3D空间推理能力，无法保证几何或动力学可行性。

Method: 基于混合状态树统一表示符号和数值状态，嵌入动力学约束由现成运动规划器和物理模拟器验证，使用VLM指导TAMP解决方案探索并根据状态视觉渲染回溯搜索。

Result: 在模拟和真实世界实验中，相比传统和基于LLM的TAMP规划器，平均成功率提高32.14%-1166.67%，复杂问题规划时间减少。

Conclusion: 该动力学TAMP框架通过混合状态表示和VLM指导，有效解决了传统TAMP方法的采样效率问题，显著提升了规划性能和可行性保证。
...
### [147] [Adaptive Trajectory Refinement for Optimization-based Local Planning in Narrow Passages](https://arxiv.org/abs/2510.26142)
*Hahjin Lee,Young J. Kim*

Main category: cs.RO

TL;DR: 提出自适应轨迹优化算法，通过路径段级安全测试和姿态级安全校正，解决移动机器人在狭窄通道中的轨迹规划问题，显著提高成功率和规划速度。


<details>
  <summary>Details</summary>
Motivation: 移动机器人在复杂环境中的轨迹规划面临挑战，特别是在狭窄通道中，传统方法常常失败或生成次优路径。

Method: 算法包含两个主要阶段：1) 路径段级安全测试，递归细分风险路径段消除碰撞风险；2) 姿态级安全校正，基于穿透方向和线搜索确保每个姿态无碰撞且远离障碍物。

Result: 仿真结果显示，相比最先进方法，成功率提高1.69倍，规划时间加快3.79倍。真实世界实验证实机器人能安全通过狭窄通道并保持快速规划性能。

Conclusion: 所提出的自适应轨迹优化算法能有效解决狭窄通道中的轨迹规划问题，显著提升成功率和效率。
...
### [148] [Self-localization on a 3D map by fusing global and local features from a monocular camera](https://arxiv.org/abs/2510.26170)
*Satoshi Kikuch,Masaya Kato,Tsuyoshi Tasaki*

Main category: cs.RO

TL;DR: 提出了一种结合CNN和Vision Transformer的新方法，用于在存在动态障碍物的情况下实现基于单目摄像头的3D地图自定位，相比现有方法在精度上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 使用廉价单目摄像头在3D地图上实现自定位对于自动驾驶至关重要。基于摄像头的自定位通常使用CNN提取局部特征，但当存在行人等动态障碍物时，CNN效果不佳。

Method: 将CNN与Vision Transformer相结合，CNN擅长提取局部特征，而Vision Transformer擅长提取全局特征，能够捕捉图像中所有patch之间的关系。

Result: 在包含动态障碍物的CG数据集上，精度提升率比无动态障碍物时高1.5倍；在公共数据集上，自定位误差比现有最佳方法减少20.1%；机器人使用该方法平均定位误差为7.51cm，优于现有方法。

Conclusion: 提出的CNN与Vision Transformer结合方法能够有效应对动态障碍物干扰，显著提升自定位精度，为自动驾驶应用提供了更可靠的解决方案。
...
### [149] [PHUMA: Physically-Grounded Humanoid Locomotion Dataset](https://arxiv.org/abs/2510.26236)
...
<details>
  <summary>Details</summary>
Motivation: 现有运动模仿方法依赖稀缺昂贵的动作捕捉数据集（如AMASS），限制了可扩展性和多样性。基于互联网视频的方法（如Humanoid-X）存在物理伪影问题，影响稳定模仿。

Method: 通过数据精选和物理约束重定向技术，强制执行关节限制、确保地面接触、消除脚部滑动，生成大规模且物理可靠的运动数据。

Result: 在未见运动模仿和骨盆引导路径跟随两种条件下，PHUMA训练的策略均优于Humanoid-X和AMASS，在多样化运动模仿方面取得显著提升。

Conclusion: PHUMA数据集成功解决了运动模仿中的物理伪影问题，为基于视频的人形机器人运动学习提供了高质量、大规模的数据基础。
...
### [150] [Thor: Towards Human-Level Whole-Body Reactions for Intense Contact-Rich Environments](https://arxiv.org/abs/2510.26280)
*Gangyang Li,Qing Shi,Youhao Hu,Jincheng Hu,Zhongyuan Wang,Xinlong Wang,Shaqi Luo*

Main category: cs.RO

TL;DR: 提出Thor人形机器人框架，通过力自适应躯干倾斜奖励函数和分层强化学习架构，显著提升人形机器人在接触丰富环境中的力交互能力，在Unitree G1上实现高达167.7N的拉力表现。


<details>
  <summary>Details</summary>
Motivation: 解决人形机器人在接触丰富环境中保持全身稳定性并进行强交互时，难以产生类似人类的适应性反应这一挑战。

Method: 设计力自适应躯干倾斜奖励函数，采用将上体、腰部和下体解耦的强化学习架构，各组件共享全身全局观测并联合更新参数。

Result: 在Unitree G1上实现后退时峰值拉力167.7N（提升68.9%），前进时145.5N（提升74.7%），能够拉动130N负载货架和单手开启60N防火门。

Conclusion: Thor框架有效增强了人形机器人的力交互能力，在接触丰富环境中表现出接近人类水平的适应性反应。
...
TL;DR: AgriGS-SLAM是一个结合视觉-LiDAR的SLAM框架，用于果园环境下的实时3D场景理解，能够在季节性变化和风动干扰下提供稳定重建和轨迹估计。


<details>
  <summary>Details</summary>
Motivation: 解决果园环境中自主机器人面临的挑战：重复行几何、季节性外观变化和风驱动叶片运动，需要实时3D场景理解能力。

Method: 结合直接LiDAR里程计和闭环检测，使用多相机3D高斯泼溅渲染，通过批量栅格化恢复被遮挡的果园结构，采用统一梯度驱动的地图生命周期管理。

Result: 在苹果和梨园的不同季节（休眠期、开花期、收获期）测试显示，相比现有3DGS-SLAM基准方法，AgriGS-SLAM提供更清晰、更稳定的重建和更平稳的轨迹，同时保持实时性能。
...
### [152] [Cooperative Task Spaces for Multi-Arm Manipulation Control based on Similarity Transformations](https://arxiv.org/abs/2510.26362)
*Tobias Löw,Cem Bilaloglu,Sylvain Calinon*

Main category: cs.RO

TL;DR: 本文提出了基于共形几何代数的多臂机器人系统协作任务空间理论框架，通过几何基元变换将复杂系统抽象为单臂系统，并推导了分析雅可比矩阵和几何雅可比矩阵，实现了经典操作空间控制技术的集成。


<details>
  <summary>Details</summary>
Motivation: 人类环境中的许多任务需要多个运动链之间的协作行为，但复杂系统的高自由度使得运动协调建模非常困难。

Method: 基于共形几何代数定义协作几何基元，通过相似变换推导系统抽象表示，并推导相关雅可比矩阵，集成到操作空间控制技术中。

Result: 在双操作臂机器人、仿人机器人和多指手的实验中展示了方法在最优控制和遥操作中的有效性，几何基元自然嵌入了零空间结构可用于次级控制目标。

Conclusion: 这项工作为协作操作控制框架奠定了理论基础，实验以抽象方式呈现，同时指出了潜在的未来应用方向。
...
### [153] [Human-in-the-loop Online Rejection Sampling for Robotic Manipulation](https://arxiv.org/abs/2510.26406)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人操作策略中应用广泛，但微调视觉-语言-动作模型时存在价值估计不准确和中间步骤监督稀疏的问题；模仿学习易于训练但性能不足。

Method: Hi-ORS通过拒绝采样过滤负奖励样本来稳定价值估计，采用奖励加权的监督训练目标提供密集的中间步骤监督，并开发异步推理训练框架支持在线人工校正。

Result: 在三个真实世界任务和两种实现中，Hi-ORS仅需1.5小时真实世界训练即可掌握接触丰富的操作，在效果和效率上大幅优于RL和IL基线方法。

Conclusion: Hi-ORS微调后的策略展现出强大的测试时扩展性，能够可靠执行复杂的错误恢复行为以获得更好的性能。
...
### [154] [RoboOS-NeXT: A Unified Memory-based Framework for Lifelong, Scalable, and Robust Multi-Robot Collaboration](https://arxiv.org/abs/2510.26536)
...
Method: 引入STEM记忆，集成空间场景几何、时间事件历史和具身配置文件，采用大脑-小脑框架，高层大脑模型进行全局规划，低层控制器本地执行。

Result: 在餐厅、超市和家庭等复杂协调任务中表现出优越性能，验证了其在异构具身环境中的有效性。

Conclusion: RoboOS-NeXT通过统一的记忆中心设计，实现了终身、可扩展和鲁棒的多机器人协作。
...
### [155] [Adaptive Inverse Kinematics Framework for Learning Variable-Length Tool Manipulation in Robotics](https://arxiv.org/abs/2510.26551)
...
<details>
  <summary>Details</summary>
Motivation: 传统机器人对自身运动学理解有限，局限于预设任务，无法有效利用工具。为了解决工具使用的四个关键要素（理解预期结果、选择合适工具、确定最佳工具方向、执行精确操作），需要开发新的方法。

Method: 通过扩展机器人逆运动学求解器，使其能够获取使用不同长度工具的连续动作序列。将仿真学习的动作轨迹与工具集成，实现从仿真到现实世界的技能迁移。

Result: 扩展的逆运动学求解器误差小于1厘米；训练的策略在仿真中平均误差为8厘米；使用两种不同长度工具时性能几乎无差异。
...
### [156] [A Sliding-Window Filter for Online Continuous-Time Continuum Robot State Estimation](https://arxiv.org/abs/2510.26623)
...
<details>
  <summary>Details</summary>
Motivation: 现有连续体机器人状态估计方法难以平衡精度和计算效率，滑动窗口方法仅限于简化的离散时间近似且缺乏随机表示，而随机滤波器方法受限于测量速度运行。

Method: 开发了一种专门为连续体机器人设计的随机滑动窗口滤波器，支持连续时间状态估计，能够在线运行且速度快于实时。
...
### [157] [Hybrid DQN-TD3 Reinforcement Learning for Autonomous Navigation in Dynamic Environments](https://arxiv.org/abs/2510.26646)
*Xiaoyi He,Danggui Chen,Zhenshuo Zhang,Zimeng Bai*

Main category: cs.RO

TL;DR: 本文提出了一种分层路径规划与控制框架，将用于离散子目标选择的高级DQN与用于连续驱动控制的低级TD3控制器相结合，在动态和部分可观测环境中实现了更高的成功率和样本效率。
...
Method: 采用分层架构：高级模块使用DQN进行行为选择和子目标选择，低级模块使用TD3执行平滑速度控制。设计了实用的奖励塑造方案和安全门机制。

Result: 在ROS + Gazebo环境中评估显示，相比单一算法基线和基于规则的规划器，该系统在成功率和样本效率方面都有提升，对未见过的障碍物配置具有更好的泛化能力，并减少了突变的控制变化。

Conclusion: 分层DQN-TD3框架在复杂导航任务中表现优异，结合了离散决策和连续控制的优势，为机器人路径规划提供了有效的解决方案。
...
### [158] [Heuristic Adaptation of Potentially Misspecified Domain Support for Likelihood-Free Inference in Stochastic Dynamical Systems](https://arxiv.org/abs/2510.26656)
*Georgios Kamaras,Craig Innes,Subramanian Ramamoorthy*

Main category: cs.RO

TL;DR: 本文提出三种启发式LFI变体（EDGE、MODE、CENTRE）来解决机器人中似然自由推理的支持集错误设定问题，通过在推理过程中自适应调整支持集，提高了参数推断和策略学习的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统LFI方法假设固定的支持集，但错误设定的支持集会导致次优且虚假确定的后验分布。本文旨在解决这一支持集错误设定问题。

Method: 提出三种启发式LFI变体：EDGE、MODE和CENTRE，每种方法以不同方式解释后验模式在推理步骤中的偏移，并在LFI步骤中同时调整支持集和后验推理。

Result: 在随机动态基准测试中验证了启发式方法的有效性；在动态可变形线性物体操作任务中，实现了更精细的长度和刚度分类，使用得到的后验作为领域分布进行基于模拟的策略学习，获得了更鲁棒的物体中心智能体性能。

Conclusion: 提出的支持集自适应方法能够有效解决LFI中的支持集错误设定问题，提高参数推断精度和策略学习性能，特别是在复杂机器人操作任务中表现优异。
...
### [159] [Hybrid Consistency Policy: Decoupling Multi-Modal Diversity and Real-Time Efficiency in Robotic Manipulation](https://arxiv.org/abs/2510.26670)
*Qianyou Zhao,Yuliang Shen,Xuanran Zhai,Ce Hao,Duidi Wu,Jin Qi,Jie Hu,Qiaojun Yu*

Main category: cs.RO

TL;DR: 提出混合一致性策略(HCP)，通过短随机前缀和一步一致性跳跃来平衡多模态行为和快速采样，在保持高精度的同时显著降低推理延迟。
...
Result: 在仿真和真实机器人上，HCP仅需25步SDE加一步跳跃即可接近80步DDPM教师的精度和模式覆盖度，同时显著降低延迟。

Conclusion: 多模态行为不需要缓慢推理，切换时间将模式保持与速度解耦，为机器人策略提供了实用的精度效率权衡。
...
### [160] [Running VLAs at Real-time Speed](https://arxiv.org/abs/2510.26742)
*Yunchao Ma,Yizhuang Zhou,Yunhuan Yang,Tiancai Wang,Haoqiang Fan*

Main category: cs.RO

TL;DR: 本文展示了如何在单个消费级GPU上以30Hz帧率和最高480Hz轨迹频率运行pi0级多视角VLA，实现了之前被认为大型VLA模型无法完成的动态实时任务。
...
Method: 引入一系列策略来消除模型推理中的开销，包括优化推理过程，并提出完整的流式推理框架用于VLA的实时机器人控制。

Result: 实验显示，采用所提策略的pi0策略在抓取下落笔任务中达到100%成功率，实现了实时性能。

Conclusion: 通过优化策略和流式推理框架，成功实现了VLA模型在消费级硬件上的实时运行，为动态机器人任务提供了可行解决方案。
